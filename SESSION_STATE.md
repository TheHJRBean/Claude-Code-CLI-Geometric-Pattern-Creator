...
- **Finding 2 — decouple Design-phase base geometry from the viewport in `usePattern`.** In Design Phase with no neighbour preview, `basePolys` + `runPIC(basePolys)` are viewport-independent yet the `useMemo` deps include `genX/genY/genW/genH`, so they recompute each 12% pan quantum. Split a base `useMemo` keyed on `config` only; the no-neighbour Design branch returns base refs directly. Composition stamping + neighbour-ghost fill genuinely depend on viewport and stay on the viewport-keyed path.

**Integrity / rollback:** `main` is the clean pre-perf state. This work is isolated on `perf/builder-render-memoization`. Each Finding committed separately so either can be reverted independently. `tsc --noEmit` green at baseline. **Verify after each commit:** `tsc` + `npm run build` + browser-check Builder pan/zoom in Design AND Composition phases, Place AND Complete modes, single- AND multi-cell, Show-neighbours on/off — the paths these changes touch.

**Risk notes:** memo bails ONLY if every prop is referentially stable — a missed unstable callback silently no-ops the optimisation (no correctness break, just no speedup). Finding 2's memo split must NOT starve the neighbour-preview / Composition paths of viewport updates (would freeze the lattice fill on pan).
