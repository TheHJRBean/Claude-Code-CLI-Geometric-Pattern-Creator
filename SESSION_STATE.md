...
3. **Answer #54 — the canonical-pose split (Opus).** Not cosmetic: **stamps currently render one image at two scales inside a single congruent class** on a curved field. Three options are written up on the issue; option 2 (chirality-symmetrised extent) fixes gradients fully but leaves stamps needing the pose fix. Wants a decision before code.
...
6. **Decoration stamps v2 (Sonnet/Opus).** v1 ships for Voids only; remaining = **Tile**-stamping, explicit mirror toggle, asset library. Memory: `project_decoration_stamps_idea.md`.
