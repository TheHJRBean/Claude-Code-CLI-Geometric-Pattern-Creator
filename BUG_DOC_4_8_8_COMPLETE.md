...
| 14 | Soft-rejection override ("Accept and continue anyway") | DONE | `544f4c3` | Some `overlaps-existing` rejections and `inside-tile` rejections are intentional overlaps the user wants to commit. Added `force` flag to `EDITOR_COMPLETE_N_GAP` action → reducer threads it into `completeNGap` (bypasses `inside-tile`) and skips the post-completion `overlapsExisting` guard. UI: when `validateMultiPick` returns either soft kind, an "Accept and continue anyway" button renders under the rejection pill. Hard rejections (`self-intersecting`, `duplicate-vertex`, `too-few`, `pick-not-selectable`, `no-real-cell-pick`) remain blocked. |
