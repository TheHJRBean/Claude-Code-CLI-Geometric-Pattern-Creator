...
- **▶ NEXT — Chunk 4 (`usePattern`)**: audited (under 1k but the most perf-fragile file — load-bearing memo deps + documented eslint-disables; perf memory warns against churn). Plan = add Std-8 tests for the pure helpers `stampSegments` + `periodicFastPathEligible` only; **DEFER** the flag-soup + 250-ln final-memo restructure. Then Chunk 5 (`reducer`), 3 (`Sidebar`), 6 (`tapratsTiling`), then Tier-2 logic (7 PIC / 8 placement / 9 decoration).
