...
1. **Browser-verify the Decoration/Stamp session (Sonnet/Opus) — 4 unverified fixes, all reachable in one pass.** Load a saved pattern with curves on, go Decoration → Stamp:
   ...
   - `2d4e504` **curved canvases** ⇒ inspector preview, exported SVG/PNG guide and Focus mode all show the CURVED silhouette; an uploaded image reaches the full curved Void with no gap at a bulge.
   ...
   - `88bd29d` **curved gradients** (same pass, same screen) ⇒ Decoration → Gradient → *This shape*, on a field with curves on. Paint a curved Void: the wash should reach the outermost bulge instead of banding to flat end-colour just short of it. Open **Focus mode** — the silhouette must be the CURVED one. Then flatten the curve and repaint: the gradient must visibly re-fit (before, it did not move at all).
2. **Answer #54 — the canonical-pose split (Opus).** Not cosmetic: **stamps currently render one image at two scales inside a single congruent class** on a curved field. Three options are written up on the issue; option 2 (chirality-symmetrised extent) fixes gradients fully but leaves stamps needing the pose fix. Wants a decision before code.
...
7. **Open bugs (unscheduled).** Force-overlapped Tiles emit their own Strands (decided 2026-06-14: make it a **toggle**, default self-contained). Memory: `project_overlap_tiles_strand_bug.md`.
