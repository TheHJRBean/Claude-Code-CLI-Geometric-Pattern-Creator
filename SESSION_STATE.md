...
**User feedback (2026-06-05): hex saved config still slow.** "Both" phases slow; **Design + Show-neighbours exceptionally slow**; lag proportional to zoom, slow even at close zoom, zoom gesture itself laggy. Diagnosis: this is the two DEFERRED structural levers, not a regression — (A) **periodicity-aware PIC** (compute: Composition + neighbour-strands re-run PIC over the whole stamped/ghost field every pan step), and (B) **viewBox→transform pan/zoom** (paint: the SVG re-rasterises the whole scene each frame; scales with visible vector count). Findings 1+2 only cleared the React-side wins (Design/no-neighbour pan is now compute-free).

**Parameter-UI lag: deferred config tried `555ccfd` then REVERTED `66a13b5`.** User: "lag extends to the parameter UI and response to changing them." Tried `useDeferredValue(config)` in `usePattern` to unblock edits — but it killed the live slider preview: during a continuous angle-slider drag the recompute can't keep pace, so React kept deferring and the preview only updated on **release**. Live preview > smooth thumb here, so parameter edits MUST stay synchronous. Reverted; left a comment at the Canvas call site so it isn't re-attempted. **Lesson: don't defer config — the real fix for parameter-edit cost is cheap recompute (lever A periodicity-PIC), not deferral.** Pan/zoom (viewTransform) deferral unaffected.

**Added diagnostic HUD `1c6e9d0`** to bisect compute vs paint before doing the risky refactor: `utils/perf.ts` (gated sink, `localStorage.perf='1'` or `?perf`), timing in `usePattern` (picMs + counts) + `StrandLayer` (strandMs), `components/PerfHud.tsx` overlay (counts + live FPS). Read: high pic/strand ms ⇒ build lever A; low ms + low FPS while panning ⇒ build lever B. **Next:** user reports HUD numbers → implement the indicated lever behind a flag (old path as fallback) on this branch. HUD is purely additive, off by default, delete with its call sites when done.

**⏸ PAUSE POINT (2026-06-05) — Builder perf, consolidated.** Full detail + two-axis cost model + bench + lever specs in `memory/project_builder_performance_idea.md` (canonical). State: on branch `perf/builder-render-memoization` (off `main`, UNMERGED — `main` is the clean rollback), SHIPPED Findings 1+2, diagnostic HUD, and **Lever A periodicity fast-path (flagged, proven exact)**. Deferring config = DEAD END (broke live preview, reverted).

**Bench finding:** Composition is compute-bound and `buildStrands` > `runPIC` at scale (567 polys → 15ms PIC + 28ms strand = 43ms/regen, both per edit). **Lever A `01e8dfb`:** render ONE fundamental domain (PIC + buildStrands on base patch only, reuse `editorBase.baseSegments`) tiled via SVG `<use>`; gated to exact+seamless cases (single-cell, pure-translation, no vertex-lines/frame/boundary-lattice), else exact fallback. Flag `localStorage.perfPeriodicity='1'`, OFF by default. Proven exact: `editor/compositionPeriodicity.test.ts` (periodic == full-field PIC, 0 missing/0 extra).

**Next session pickup:** (1) **browser-verify Lever A** on the hex — `localStorage.perfPeriodicity='1'`, Composition phase, check perf win (HUD `composition·periodic`, picMs/strandMs≈0) + visuals identical; (2) generalise Lever A (rotated/triangle, multi-cell, vertex-lines, frame, + apply same `<use>` tiling to Design Show-neighbours ghost field) — each gate listed in the memory; (3) Lever B viewBox→transform (paint axis); (4) browser-verify Findings 1+2; (5) merge → main. Branch order: `1e44950` F1 → `aa42d66` F2 → `1c6e9d0` HUD → `555ccfd`→`66a13b5` deferred-config+revert → `01e8dfb` Lever A → docs. All tsc + 203 vitest + build green.

**Integrity:** `main` untouched. Each step a separate commit, independently revertable. **Verify paths for Findings 1+2:** Builder pan/zoom in Design AND Composition, Place AND Complete, single- AND multi-cell, Show-neighbours on/off. Memo bails ONLY if every prop is referentially stable (a missed callback silently no-ops the speedup — no correctness break).

---

**Current branch:** `perf/builder-render-memoization` (off `main`; the Builder-perf work above). Prior feature work (framing, flexible placement, Gallery frame, multi-cell, vertex placement — entries below) landed on `main`.
