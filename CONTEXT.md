...
The Decoration-Phase operation that marks a **Junction**. A **dot** or **star** is a figure placed *on* the crossing; a **twinkle** is the crossing's corners **rounded off** — a fillet in each wedge between adjacent arms, tangent to both and running a settable **reach** up each one, so the **Strands** read as swelling into each other (colour it like the Strands) or as an accent web across the corners (contrast it). Any of them can be **hollow** (drawn as an outline) with its own fill inside. The third Decoration target after areas (**Void Fill**) and lines (**Strand colour**): the *points* of the arrangement. Sized as a multiple of the Strand width, so it tracks the line work. Binds at a **Grouping scope** rung like any paint (**All** / **Matching** / **Repeat** / **Single** — there is no **Twins** rung, because that key hashes a target's outline and a Junction is a point). Drawn above the Strands and included in exports. v1 applies to **solid** Strands only; a divided (`lines`) stroke withdraws the target rather than covering its own divisions.
