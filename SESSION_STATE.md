...
**Latest (2026-08-03, Void seams + a canvas background control): SHIPPED, ✅ BROWSER-VERIFIED (headless).** User: *"With strand colour off there are still very small unpainted pixels between voids. Improve the void geometry and add background painting option (or tell me where it is if I missed it)."*
...
**Previous (2026-08-03, legacy Frame seeds on the view): FIXED, ✅ BROWSER-VERIFIED (headless).** Bug capture: *"stamp and void fill removed by frame"* — legacy substrate, floret-pentagonal, Decoration, `frame: shape/square/400/clip`, 3 Void fills + 1 Stamp still in the config. **Not a decoration bug at all.** The filed config renders perfectly: fills and stamps sit INSIDE the frame-clipped group (`PatternSVG.tsx:309-310`), and on this substrate the Frame touches nothing upstream of render — the Void extraction bound is the quantised **viewport**, never the frame outline (`usePattern.ts:831`), and `SET_GALLERY_FRAME` only writes `state.frame`. Four headless scenarios (config as filed ± frame, paint-then-frame, frame-then-paint, Export SVG) all kept 65 fill paths and 15 stamp images.
