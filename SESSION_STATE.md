...
- **Chunk 3 DONE** (branch `review/sidebar`, not yet merged): `Sidebar.tsx` (907) under 1k. TWO moves this time (production code changed → needs Wave-D browser-verify): (1) Std-2/6 — extracted a `<Section>` wrapper deduping the section chrome repeated 8× (907→890; My Patterns left raw on purpose — its spacer sits outside the open-gate); (2) Std-8 — extracted the frame units↔px clamp to pure `frameUnitModel`/`frameUnitsToPx` in `frame.ts` + **15 tests** (367→382) incl. a parametric round-trip-stability guard for the documented slider-freeze bug. Behaviour-preserving (`<Section>` = verbatim chrome; frame math pinned). tsc + 382 vitest + build green; bundle JS 419.71→418.96 kB. Ledger has the Chunk-3 section + result.
- **▶ NEXT — merge Chunk 3 to `main`**, then Chunk 6 (`tapratsTiling` 796 — has tests, Std-0/4 adversarial coverage), then Tier-2 logic (7 PIC / 8 placement / 9 decoration). ⏳ Wave-D browser-verify owed: Sidebar collapse/expand every section + toggle each control + Frame size slider to max/min (no freeze).
