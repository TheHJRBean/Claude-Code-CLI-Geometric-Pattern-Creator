...
**🧹 2026-06-13 — thermo-nuclear whole-codebase review IN PROGRESS.** Driven by `THERMONUCLEAR_REVIEW_PLAN.md`; live ledger + metrics in `THERMONUCLEAR_REVIEW_FINDINGS.md`. **Wave A done** (green baseline pinned). All work below is MERGED to `main` + pushed; working tree clean, no open review branches; 328 tests / tsc / build green.
...
- **Chunk 5 DONE** (branch `review/reducer`, not yet merged): `reducer.ts` (877) under 1k. Mega-switch→handler-map **REJECTED** (idiomatic type-safe switch; conversion loses payload narrowing + adds indirection, the skill's "unnecessary wrapper" anti-pattern). Banked the Std-8 gap instead: **25 characterization tests** (`figureMutations.test.ts`, 342→367) pinning the edge↔vertex line mutual-exclusion invariant, the figure/curve mutation helpers (decouple seeding, curve defaults/clamp/merge, target isolation, RESET_FIGURES), immutability, editor no-op guards, and the multi-cell boundary-scale lattice invariant. **Zero production-code change.** tsc + 367 vitest + build green, bundle unchanged. Ledger has the Chunk-5 section + result.
- **▶ NEXT — merge Chunk 5 to `main`**, then Chunk 3 (`Sidebar` 907 — pure-UI, likely a control-registry Std-2/4 candidate + manual-verify), 6 (`tapratsTiling` 796 — has tests, Std-0/4 adversarial), then Tier-2 logic (7 PIC / 8 placement / 9 decoration).
