...
| 19.3 | ✅ **DELIVERED** — Decoration phase (a/b/c) + Paint mode (Off·Voids·Strands) + hover highlight + click-apply + bulk fill + undo | user-verified "functioning broadly as intended"; perf-tuned for multi-cell (fast-path tiling, no per-pan extraction). Minor: first-paint one-time lag. ⚠️ **The "undo" here was only half-true until `65d09d9` (2026-08-04)** — the allowlist entries existed but the snapshot covered only the Patch, so Stamps/gradients never undid at all and on a **legacy substrate** one Ctrl+Z after one paint *cleared the Lab*. Snapshot is now a `{ editor, decoration }` pair; **Clear paint** joined the Undo/Redo row |
