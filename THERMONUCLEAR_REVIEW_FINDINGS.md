...
### Chunk 13 — cross-cutting duplication & canonical-helper sweep (Std-6)

The final whole-codebase pass, done last so it has every file's context. Two clean canonical-helper consolidations + the owed `computeCurves` test landed; the PIC branch-ladder reframe is **explicitly deferred to its own session** (the plan + SESSION_STATE both prescribe this — it's the program's highest-risk move, gated on a preserve-vs-fix product decision).

| Chunk | File | Sev | Finding | Remedy | Status |
| --- | --- | --- | --- | --- | --- |
| 13 | `state/reducer.ts` + `editor/exposedEdges`/`placement`/`vertexPlacement`/`tileTypeId` | S6 | `centroidOf` (reducer) re-implemented the canonical `centroid`; `interiorAngle` (placement) and `tileInteriorAngleAt` (vertexPlacement) were byte-identical, and `interiorAngles` (tileTypeId) repeated the same acos formula inline. | **DONE**: `centroidOf`→canonical `centroid`. New pure `polygonInteriorAngleAt(poly, i)` in `utils/math` (operation order preserved — it feeds the `tileTypeId` signature hash); shared `tileInteriorAngleAt(tile, verts, i)` in `exposedEdges` wraps it with the regular `(n-2)π/n` shortcut; placement + vertexPlacement + tileTypeId all delegate. tile-type signatures unchanged (the existing tileTypeId tests stay green). | done |
...
`tsc` clean · **549/549 tests pass** (539 +10 `computeCurves` characterization) · build OK · bundle JS **419.83→418.94 kB** (gzip 127.56→127.40). Two canonical-helper dedups (centroid/interior-angle + the 5 rotation-matrix sites → `rotate`), net **−36 production lines**, all byte-equivalent and guarded by the existing + new tests. The owed `computeCurves` test is banked. **The PIC branch-ladder reframe is deferred to its own focused session** with rationale — that's the disciplined call the program prescribes, not a punt. **user-verified: n/a** for the dedup (no observable change; metrics are the deliverable). With this, the standing-code review program is complete **except** the deferred PIC reframe + the accumulated browser-verifies.
