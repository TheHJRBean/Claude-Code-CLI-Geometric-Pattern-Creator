...
**2026-06-01 — Flexible placement + skippable overlap warning (Design Phase + Place mode).** Replaced hard placement scoping (the `viableSidesFor*` picker filters) with flexible placement + a skippable overlap warning, mirroring the multi-vertex Complete `force` pattern. Design rationale + full per-slice status in `memory/project_flexible_placement_idea.md` (**canonical**). Three slices: **`b78945f`** — edge + vertex viability probes now use the shared edge-cross `overlapsExisting` (catches partial-overlap false-negatives); `VertexOrientation` gains `overlaps`, `vertexPlacementOrientations` emits overlapping orientations tagged; new `placeableSidesForVertex`. **`605a483`** — `force?` on the 3 placement actions + reducer + all 3 orbit placers (skip overlap gate on force, keep structural resolution; symmetry orbit-mate collisions now caught accurately). **`904f859`** — picker shows ALL `PICKER_SIDES`; overlaps badged dashed-amber + ⚠ → "place anyway" dispatches `force:true`; vertex page-2 flags per-orientation overlap; `EditorPickerOverlay` gains `forceableSides`. `tsc` + build green. **Not browser-verified** — verify edge/vertex/section overlap-force with symmetry on + off. Known limits: vertex sizes with no angularly-fitting sector stay disabled; warning text generic (no distinct orbit-mate message).
