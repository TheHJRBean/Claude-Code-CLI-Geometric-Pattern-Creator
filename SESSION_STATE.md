...
**Also 2026-08-05 (gradient stop cap 8 → 16): SHIPPED + ✅ HEADLESS-VERIFIED.** User: *"Increase the number of control points I can have in a gradient"*. One constant — `GRADIENT_MAX_STOPS` in `types/editor.ts` — and it was **UI-only**: `migrations.ts` validates a min of 2 and no maximum, so raising it invalidates nothing and a save authored at 16 already loaded fine. Stop-bar markers thin from 12 px to 8 px past 10 stops so they stay individually grabbable on a sidebar-width track, `+ Stop`'s tooltip names the cap, and the readout now leads with a live `n/16` count so the ceiling is visible before the button greys out. Verified in the browser (`scripts/verify/gradientStops.mjs`) on the across-frame surface: panel reaches **16/16**, `+ Stop` disables, and the rendered `<linearGradient>` def in the pattern SVG carries **16 `<stop>` elements** — the check is on the rendered defs, not the panel text. Spec updated (DECORATION_GRADIENTS_SPEC decision 2).
