...
- **#1 ENTRY TIMEOUT — FIXED (`c68ef85`).** Entering Decoration hung (O(n²) extraction over the padded gen rect). Fix: spatial-grid broad-phase in `extractVoids` (short segs bucketed by bbox cells, few long segs brute-forced; identical results) + bound extraction to the VISIBLE viewport not the 0.75×-padded rect. Probe (4.8.8, 3859 segs): straight 20–78 ms (was multi-second hang), curved 30872 flat segs 580 ms (was minutes). **Still deferred (not blocking):** the periodicity shortcut (one fundamental domain + classify by signature) for further speedup, esp. curved zoomed-out (~580 ms); perf-gate hover→first-click fallback; drag-pan-over-Void swallowed by pointerdown.
