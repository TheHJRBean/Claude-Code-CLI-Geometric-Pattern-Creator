...
**Latest (2026-08-04, download filenames follow the config): SHIPPED (tests + tsc green, ⚠️ NOT browser-verified).** User: *"I want downloaded files to save with the name of the saved config, if there is one"*, then *"Any config with a designated name should download with that name. The saved name should take priority."* Every export was hardcoded `islamic-pattern.{svg,png,json}` — a folder of downloads was indistinguishable.
