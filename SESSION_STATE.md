...
**🧹 2026-06-13 — thermo-nuclear whole-codebase review IN PROGRESS.** Driven by `THERMONUCLEAR_REVIEW_PLAN.md`; live ledger + baseline metrics in `THERMONUCLEAR_REVIEW_FINDINGS.md`. **Wave A (guardrails) done** — green baseline pinned (tsc clean, 315 tests, build OK). **Chunk 1 done on branch `review/tessellation-lab-mode-split`** (`039502a`): `TessellationLabMode.tsx` 2243→812 ln; `EditorDesignControls` god-component split into `src/components/lab/` (labShared, EditorDesignControls orchestrator, CompositionPanel, DecorationPanel, FramePanel, DesignPanel — no file >1000). Pure verbatim JSX relocation, behaviour unchanged. **⏳ NEXT: Wave-B Verification Checkpoint with user** — open Lab, walk Design→Composition→Decoration, place a tile (edge/section/vertex), Complete a gap, switch active cell, undo/redo, toggle Frame controls. Confirm identical behaviour, then merge to main. Remaining Chunk-1 open finding: shared pill-button style helper (S3/S4, deferred). **Bug found during verify (NOT refactor-caused — placement logic untouched):** Place mode doesn't consistently offer inside-vs-outside placement on a seed-tile edge → memory `project_seed_tile_place_inside_outside_bug.md`, fold into review Chunk 2 (overlay cluster). **Bug 2 (also pre-existing, PIC layer):** force-overlapped tiles sometimes make the overlap region emit its own Strands as if it were a new tile (inconsistent); want self-contained per-tile figures only → memory `project_overlap_tiles_strand_bug.md`, fold into review Chunk 7 (`pic/`).
