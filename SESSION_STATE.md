...
> **Metrics snapshot (2026-06-13):** tests **315 → 405** (+90 across chunks 2/3/4/5/6). Largest-file table (vs Tier-0 baseline): TessellationLabMode 2243→812, Canvas 974→922, Sidebar 907→890, reducer 877 (unchanged), usePattern 902 (unchanged), tapratsTiling 796→800. Bundle JS 418.89 kB (gzip 127.23). No file >1000. FPS not re-measured this session (no perf-touching change).
>
...
- **Chunk 6 MERGED** (`<pending hash>`): `tilings/tapratsTiling.ts` (796, ~540 of which is the `TAPRATS_DATA` literal) under 1k → Std-0/4/8 audit, not a restructure. TWO behaviour-preserving production fixes: (1) Std-0 — deleted dead code in `intersectsViewport` (the per-vertex pre-check is provably subsumed by the AABB-overlap test); (2) Std-5/8 — added a `edgeLen > 0` fail-closed guard (a 0/NaN scale zeroed the lattice vectors → `ceil(diag/0)=Infinity` → infinite hang; reachable via crafted/legacy LOAD_CONFIG). Plus the missing adversarial layer: **+23 tests** (382→405) — data-integrity sweep across all 12 tilings (`sides===vertices.length`, finite coords — catches hand-entered data typos), determinism, MAX_POLYGONS cap, degenerate viewport, straddle-inclusion, `getTapratsTileTypes`. Deferred: splitting `TAPRATS_DATA` to its own module (pure relocation, no concept reduction). File 796→800 (comments+guard). tsc + 405 vitest + build green.

- **▶ NEXT — Tier-2 logic (the harder half begins).** Chunk 7 (`pic/index.ts` 667 + `pic/` package — the documented edge-case **minefield**: `emitStarArms`/`pairAtVertex` branch priority, edge-slide, per-ray fallback). **Characterization tests are MANDATORY before touching `emitStarArms`** (skill + plan + memories `project_pic_irregular_polygon_bugs` / `feedback_pic_pair_selection`). Folds in the OPEN `project_overlap_tiles_strand_bug` (force-overlapped tiles emit overlap-region strands). The code-judo prize: can the ordered-`if` branch ladder become a named-case policy table? Add adversarial tests for the borderline irregular polygons FIRST, confirm they pass on current behaviour, only then consider restructure. Cold-start recipe: branch `review/pic`, per-chunk checklist in `THERMONUCLEAR_REVIEW_PLAN.md` §4. **Treat any refactor here as high-risk.** Then Chunk 8 (`editor/` placement family), 9 (`decoration/`); then Tier-3 test-free zones (10 rendering / 11 components / 12 types) + Tier-4 dup sweep (13).
  - ⚠ **Expectation reset for Tier-2:** unlike chunks 3–6 (all under 1k, mostly test-banking), `pic/index.ts` is genuine spaghetti with real refactor payoff AND real risk. This is where the program's headline "latent PIC edge-case bugs fixed" (Wave-E checkpoint) is earned — but ONLY behind characterization tests. Do not refactor `emitStarArms` blind.
