...
**Parameter-UI lag fix `555ccfd` (deferred config).** User: "lag extends to the parameter UI and the response to changing them." Cause: pan/zoom was deferred but config edits were not — changing a figure param re-ran the whole pipeline (PIC over the stamped field in Composition) synchronously on every slider tick, blocking the slider + canvas (same render). Fix: `usePattern` now reads `useDeferredValue(config)`; sidebar sliders + overlays keep LIVE config so they stay responsive while geometry catches up at low priority (lags edits by a frame or two). Mirrors the viewTransform deferral. Per-recompute throughput unchanged (that's lever A); this just unblocks input. tsc + 201 vitest + build green. ⏳ browser-verify.
