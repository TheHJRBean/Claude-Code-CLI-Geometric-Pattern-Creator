...
**✅ Lever A browser-verified (2026-06-05, user) — CORRECTED:** with the flag on, the strand angle slider **renders in real time but is STILL laggy** (not smooth), and **overall pan/zoom perf is not much improved.** So Lever A's compute win is real but doesn't translate to felt smoothness — **the dominant remaining bottleneck is the PAINT axis (Lever B), not compute.** (A global angle change repaints every `<use>` clone each frame; `<use>` cut compute + DOM, not paint.) Next priority is **Lever B**, NOT generalising Lever A.

**Next session pickup (reordered after the verify):** (1) **Lever B — pan/zoom off the SVG `viewBox`** (the real remaining win; baseline lag ∝ on-screen vector count): GPU-composited CSS transform on a wrapper during the gesture, commit to viewBox on gesture end; touches `worldToScreen` + picker positioning + the rotation `<g>`; build behind a flag. (2) THEN generalise Lever A if still warranted (multi-cell, ghost field, etc. — gates in the memory). (3) browser-verify Findings 1+2. (4) merge → main. Branch order: `1e44950` F1 → `aa42d66` F2 → `1c6e9d0` HUD → `555ccfd`→`66a13b5` deferred-config+revert → `01e8dfb` Lever A → docs. All tsc + 203 vitest + build green.
