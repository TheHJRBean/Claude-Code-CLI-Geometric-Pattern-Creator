...
2b. **#55 — no way to add or edit a Frame on a Gallery preset (Sonnet).** New user report, traced but not started; see the entry above. **First step is a question, not code:** did they load via the Lab's Presets shelf (which converts to a Patch and should already work) or the Gallery browser's "Edit in Lab" (legacy substrate, genuinely has no Frame UI)? If the former, the bug is elsewhere and the trace above is the wrong tree.
