...
**🔧 2026-06-10 — 19.4 snag fixes in flight.** Perf-review resume (post-CLI-crash; nothing was lost, `017a7f9`+`8eb6abe` landed). Review found: (1) known snag — every paint re-runs base PIC + void extraction (`editorBase` keyed on whole `config`, `decorationFills` too); (2) **bug** — multi-cell + `alternateOrientation` blanked painted fills + killed Voids paint (render fast-path fires — stamps stay rotation-0 on a rotated basis — but `decorationFills` bailed on the flag); (3) gate mismatch wastes extraction where the fast-path never fires (triangle rotation-π intra-stamp; boundary-lattice overlay on). **Fix #2 committed** (drop the `alternateOrientation` bail — extraction is rotation-consistent). Next: #1+#3 — re-key `editorBase` on geometry sub-fields (`tiling.type`, editor `cells`/`activeCellId`/`edgeLength`/`configuration`/`alternateOrientation`, `figures`, `figureRouting` — confirmed full runPIC read-set; flatten adds `smoothTransitions`), split `decorationFills` into geometry-keyed reps + decoration-keyed colouring, shared fast-path-eligibility predicate for both gates. Reducer paint actions confirmed to preserve `cells` ref. ⏳ browser-verify alternate-orientation decoration after.
