...
**Line division** _(code: `lineStyle: 'lines'`, `lineCount`)_:
How many parallel lines one stroke is drawn as — a property of the **Strand** and, in the same vocabulary, of the **Frame** border. The stroke keeps its width and is divided within it, so raising the count thins the lines rather than widening the stroke. `n` divisions produce `n` **lines** and `n − 1` **stroke gaps**, interleaved from the outside in; a **ring** is one radial position (a band and its mirror across the centreline), and a **grain** is which of those a colour is applied at (All / Matching / Individual). Both bands take colours, but an unfilled one differs: a stroke gap is *cut out* and shows what is behind, an unfilled line takes the stroke's own colour.
_Avoid_: double / triple (withdrawn hard-coded styles — they are now a count plus a line/gap ratio), stripe, ply

**Stroke gap** _(code: `gapFills`, `innerFill`)_:
The space between two adjacent parallel **lines** of a divided stroke. **The third and last sense of "gap" in this project, and the only one that is not a region of the plane** — the Design-phase *gap* is un-tiled area inside a **Cell** that **Complete** fills with **Tiles**, and a **Void** is a face of the strand arrangement. Say "stroke gap" wherever a sentence could be read as either; a bare "gap" inside the divided-stroke controls is unambiguous and stays.
_Avoid_: void (a face of the arrangement, not a slot in a stroke), space, channel
...
_Avoid_: gap (the Design/Complete tile-gap, or a **stroke gap** between a divided stroke's lines — neither is a Void), cell/region/interstice (rejected — the canonical noun is **Void**), background (the canvas backdrop is `StrandStyle.background`, not a Void)
