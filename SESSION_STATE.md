...
- **Chunk 4 DONE** (branch `review/usePattern-tests`, not yet merged): `usePattern` audited — under 1k but the most perf-fragile file (load-bearing memo deps + documented eslint-disables; perf memory warns against churn). Restructure (flag-soup + 250-ln final-memo) **DEFERRED with rationale** (no Std-1 payoff, needs a dedicated perf-verify session). Banked the missing Std-8 net instead: exported the two pure helpers `stampSegments` + `periodicFastPathEligible` and pinned them with **14 characterization tests** (328→342). No production logic changed. tsc + 342 vitest + build green, bundle gzip 127.21 (no regression). user-verify n/a (no observable change). Ledger has the Chunk-4 section + result.
- **▶ NEXT — merge Chunk 4 to `main`**, then Chunk 5 (`reducer`), 3 (`Sidebar`), 6 (`tapratsTiling`), then Tier-2 logic (7 PIC / 8 placement / 9 decoration).
