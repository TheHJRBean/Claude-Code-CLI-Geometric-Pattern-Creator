...
**⏳ Pending at session end:** browser-verify the framing changes (relocation + frame-node completion + Design-phase no-clip neighbour strands); not yet merged to `main` (lives on `feat/art-deco-egypt-theme-revamp` alongside flexible-placement + Gallery-frame work, all unmerged).
