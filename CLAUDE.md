...
2. `pic/stellation.ts` — computes 2 **Contact Rays** per polygon edge at ±(π/2 − θ) from the edge direction. Per-edge-θ variants (`computeContactRaysPerEdge` / `computeVertexRaysPerVertex`) serve the **Morph** (Step 20): with an active `config.morph`, `runPIC` evaluates θ per edge midpoint (and per vertex for vertex lines) through the world-space morph field (`pic/morph.ts` — gradient-stop blend, clamped band; stops' effective values = start `figures` overridden by each stop's partial overlay). A shared edge has one midpoint so both polygons agree on its θ — Strands stay straight through contact points by construction. Morphed configs are ineligible for the Lever-A `<use>` fast-path (`periodicFastPathEligible`), so PIC always runs over world-space polygons. Probe suite: `pic/morphProbe.test.ts` (also documents two *inherited* uniform-θ artifact classes the morph merely exposes: centroid-V mixed-regime kinks in branch-transition bands, and vertex-ray leaks at α > interior half-angle).
