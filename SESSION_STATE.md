...
**Three things deliberately NOT changed — (c) is an OPEN question awaiting the user.** (a) `bboxAxisAtAngle` stops the axis where it **exits** the box, so at oblique angles a corner of a non-square box projects past the end and renders flat — independent of curves, pinned by an existing test that names the behaviour, and widening it would change every Fit plus the frame/strand washes. Pinned as known in the new suite. (b) The legacy substrate's view-rect seed fallback is documented and deliberate (an infinite field has no content extent), not a bug.

(c) ⚠️ **Gentle curves are discarded from Void outlines entirely, at a sharp cliff.** `extractVoids` runs `simplifyCollinear` at a fixed **1.5°** tolerance (`voids.ts:364`) on the extracted outline. Measured on 4.8.8: **offset ≤ 0.05 ⇒ 0/59 Voids keep the curve; ≥ 0.055 ⇒ 59/59 do.** Below the cliff the strands still render bowed while the Void fill, its exported canvas and Focus mode all go straight-edged. The 1.5° simplification is *correct for the identity outline* (it stops a T-junction vertex splitting a congruent class) and wrong for the rendered one. Fix = simplify only the identity outline, leave `polygon` at full fidelity. **Not done: it changes rendered Void geometry app-wide for gentle curves, so it wants the user's go-ahead.** Not the cause of the reported bug (that was `4ba619d`), found while diagnosing it.
...
2. **Decide (c) above — the 1.5° curve cliff (Opus, small).** Gentle curves (offset ≤ ~0.05) are stripped out of Void outlines at extraction, so fills/canvases/Focus go straight-edged while strands stay bowed. One-line-ish fix (simplify the identity outline only), but it changes rendered Void geometry app-wide for gentle curves. **Wants a yes/no before code.**
