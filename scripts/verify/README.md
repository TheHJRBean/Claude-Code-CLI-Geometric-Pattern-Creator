...
| `strokeBandFills.mjs` | Per-**line** colours on Strands, on a **multi-piece** field (Frame completion, so the fast path is off): raised ceilings reach the sliders; all 3 line rings of a 5-line stroke are drawn; the ring stack is **band-major across pieces** (piece-major blotches every crossing, and one piece cannot show it); the Individual grain is offered on a Strand; each Individual band is on an **offset** path | ✅ passes |
...
| `strokeLink.mjs` | **Link stroke design** + the one-shot copies, asserted on the **canvas** band counts rather than the panel sliders (a slider reads back what you typed whether or not it reached the pattern): copy buttons at both ends, border→Strands and Strands→border both land | ✅ passes |
| `bigRasterExport.mjs` | Large PNG export: every menu resolution downloads at the requested width with ink on it (Max-fill on and off); the 8192 px raster is drawn as 2×2 tiles yet matches a single whole-canvas decode (mean \|Δ\| 0.05/255) with no seam at the boundary; and an over-budget canvas is shown dropping its draws silently — the failure the canvas probe exists to catch | ✅ passes |
