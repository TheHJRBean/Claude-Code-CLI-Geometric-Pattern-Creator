...
Status: **ALL STEPS 0–6 DONE 2026-07-14 — EPIC COMPLETE.** Step 0 spike PASSED (findings below); Steps 1–2 shipped `f42dce7` (#20/#21); Step 3 shipped `70945eb` (#22 — `src/pic/rosettePatch.ts` + tests); Step 4 shipped `09187d8` (#23 — wired into `usePattern` via the new `runPICForCategory` seam, 6 old goldens retired, smoke-tested); Step 5 shipped `ca506e9` (#24 — `figureRouting` removed end-to-end after audit confirmed it was dead in production; `pic/index.ts` branch ladder shrunk via `probePair`/named-case tables/`pushSegment`/`pushCentroidPair`, 7 goldens bit-identical). Step 6 shipped 2026-07-14 (#25 — Archimedes' Star: Step-0-style spike PASSED with **zero new tolerance work** — `runRosettePIC`'s straight-vertex branch carries the 12-gon's collinear vertices natively, 0 defects across θ 27.5–80, while `runPIC` provably breaks on the same field at θ=60 (odd-degree/lost segments) and θ=80 (594 crossings); shipped as a `TAPRATS_DATA` block (12-gon + 2×4 triangle translates, canonical edge 1) + `rosette-patch` registry entry with `tileTypes` `12`/`3`, auto-joining the rosettePatch grand matrix + interop suites and the Generator sampler). Step 3 decisions: fixed-length + vertex-lines inherit runPIC semantics (shared helpers exported from `pic/index.ts`); decagonal `6.3` residual accepted as weave, also present at θ=80 (documented test exclusion).
