...
**⏸ PAUSE POINT (2026-06-05) — Builder perf, consolidated.** Full detail + the two-axis cost model + lever specs live in `memory/project_builder_performance_idea.md` (canonical). One-line state: Findings 1+2 + diagnostic HUD shipped on branch `perf/builder-render-memoization` (off `main`, UNMERGED — `main` is the clean rollback). Deferring config = DEAD END (broke live preview, reverted — see above). **Remaining: lever A periodicity-PIC (compute — the real fix for slider lag in Composition) + lever B viewBox→transform (paint).** Both build behind a flag with the current path as fallback.

**Next session pickup:** (1) get HUD numbers on the hex (`localStorage.perf='1'`, drag angle slider while laggy → pic ms / strand ms / fps) to pick A vs B; (2) build the indicated lever flagged; (3) browser-verify Findings 1+2; (4) merge branch → main. Branch commit order: `1e44950` (F1) → `aa42d66` (F2) → `1c6e9d0` (HUD) → `555ccfd`→`66a13b5` (deferred config + revert) → docs. All tsc + 201 vitest + build green.
