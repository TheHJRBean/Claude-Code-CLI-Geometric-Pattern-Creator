...
**Next (cold start):** unchanged priority — user browser-verify backlog. For this session specifically: verify (a) Clear sits at panel top; (b) a stop deletes via well-× and via double-click on its marker, min 2; (c) drag red stop right of white → white becomes the start colour on the painted Void + in focus mode (not just the track); (d) select a washed-out stop, click × Multiply → it deepens/saturates (repeat intensifies), painted Void + focus update live. Then resume #44 verify → **#45 slice 2 (Opus)**: across-frame underlay + on-canvas handles. Localhost dev server started this session on `http://localhost:5173/` (Vite, background task).
