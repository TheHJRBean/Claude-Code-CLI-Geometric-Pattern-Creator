...
> earned. Read it before picking items 1–3. (The 1.5° curve cliff it also
> described is **done** — `a97e079`, below.)
>
> **Agreed next task (user, 2026-08-03): #32, the Girih preset reveal** — the
> last open `guides` issue now that v1 is complete. **Do not start coding it.**
> It needs a grill/enumeration pass first, and it has a real architectural fork
...
>
> **And a sixth (SHIPPED + headless-verified, `6a27ae6`..`a545979`): junction
> ornaments — dots / stars / twinkles on the Strand crossings, hollow or solid,
> with a fillable hollow, optionally in the Strand's own colour and over or
> under the line work.** A new Decoration paint target with the usual Reach
...
**Latest (2026-08-05, junction ornaments on ASYMMETRIC fields): FIXED + ✅ HEADLESS-VERIFIED (`87c16a7`).** User: *"it doesn't work on junctions that are not symmetrical. Fix that. Cairo pentagon is a good example."* (Their other remark — *"the issue was that I didn't spot the ornament on all junctions button"* — was discoverability, not a defect; **not addressed by choice**, see the RESUME block.)
  ...
  - **Saved records still resolve.** The signature **halves its arm-gap ring when the ring repeats every half turn**, which is precisely what a straight junction does, so every key on a straight field is byte-identical to the pre-arms one. *Only* the half turn — a right-angle ring is `[90,90,90,90]` and reducing to its minimal period would key it as something the old code never emitted. **The compatibility test caught that**, which is the argument for writing it.
  - **A signed zero was load-bearing.** `-d.y` of a zero `y` is `-0`, and `atan2(-0, -1)` is −π rather than +π, so which corner got emitted first depended on it — and two shipped tests were pinned to that accident. Both sorts normalise to `[0, 2π)` now.
  - Regression `src/strand/junctionArms.test.ts` runs **real PIC fields, one straight and one bent**, and pins that Cairo still bends (or the fixture would go vacuous). Verified red first — the old builder invents **6** corners on a 4-corner bent crossing — as was the new browser check (**200/200** twinkles point-symmetric before, **34/200** after; the 34 are Cairo's genuinely straight class). Tests **1795 green** (+8), tsc + build clean.
  - **The lesson generalises and is now a memo** (`feedback_symmetric_fixtures_hide_asymmetry`): every unit fixture in the shipped suite was a right-angle crossing, so all 38 of them passed while half the app's tilings drew the feature wrong.

**Previous (2026-08-05, junction ornaments — the sixth request this day): SHIPPED + ✅ HEADLESS-VERIFIED (`6a27ae6`..`a545979`).** User (as an `/idea`, then *"now implement the idea"*): *"add visual elements at strand junctions e.g. dots or stars or twinkles, these will be initially just for solid strands. They can be hollowed or not and the hollow can be filled."* Then two follow-ups off the browser pass, both shipped (bullets 5–6 below): the **twinkle** was rebuilt from the user's diagram, and ornaments gained **match the Strand colour** + **over / under**.
  ...
  - v1 is **solid Strands only** by one predicate both the renderer and the panel ask, so the control can't promise what the renderer won't draw; a divided stroke withdraws the ornaments and the panel says why.
  - **A twinkle is not a figure stamped on the junction** (`468cd68`, from the user's diagram — `Screenshots/twinkle.jpg`): it is the crossing's corners **rounded off**, a fillet in each wedge between adjacent arms, tangent to both and running a settable reach up each one — the Strands swelling into each other. A thread through a crossing continues **both** ways, so two threads give **four** arms and four corners; treating a direction as one arm would round half of them. Everything else follows from the shape being LOCAL: the placement carries the junction's `dirs`, a twinkle is never rotated (its frame is the junction's own, so the alignment controls hide for it), it can't share a `<defs>` path (one `<path>` per junction), and `size` means *reach along the arm*, not a diameter — the panel relabels it. One reach for every arm: "further up this thread than that one" would need a per-thread control with no stable way to name a thread across a congruent class.
  - **Match the Strand colour + over / under** (`a545979`). Matching resolves the **same ladder that paints the Strands** — extracted out of `StrandLayer` into `decoration/strandColour.ts` and called by both, because an ornament resolving it even slightly differently would sit on the line work in *almost* its colour, which reads worse than an obviously different one. One colour per junction: the threads meeting there can be painted differently and a twinkle's wedge is bounded by two arms that may belong to two of them, so the first thread in enumeration order wins. `'none'` — the sentinel a removed strand paint stores to hide the line work — **removes** the ornament rather than drawing it in nothing, so ornaments can't be left floating over hidden Strands. `layer` splits the placements into two groups either side of the strand `<g>`; the under layer sits **outside** the Combine seam mask, which exists to erase Rays, not ornaments.
  - Verify: `scripts/verify/junctionOrnaments.mjs` (19 checks, all pass — re-run green on pickup 2026-08-05). Tests **1787 green** (+52 over the pre-feature baseline), tsc + build clean. ⏳ **The user's own browser pass is the one thing outstanding.**
...
**Previous (2026-08-05, Frame border outside the outline + per-gap-ring fills): SHIPPED + ✅ HEADLESS-VERIFIED (`622a578`).** User: *"I want border stroke to always be outside the frame so that it doesn't encroach on the pattern. I also want to be able to fill each individual gap (toggle individual vs all) including with gradients (if that isn't completed)."* Scope agreed with the user: **both** surfaces (Strands + Frame border), **flat colours now, gradients later** → filed as **#56**.
  ...
  - **A mixed set needs its own mask.** Rings paint as concentric underlay strokes beneath the masked ink — an inner ring's colour survives exactly where the style mask cuts to it. That cannot express an *unfilled* ring, because the outer ring's stroke covers everything inside it; so when some rings are filled and others aren't, the underlays sit behind a second, black-backed mask that whites in only the filled rings (`gapFillMaskBands`). Uniform sets skip it.
  - Data: `gapFillMode: 'all' | 'individual'` + `gapFills: (string | null)[]` on **both** `StrandStyle` and `FrameStroke`, read through the same `readLineStyleFields` on both schema gates. UI is one shared `GapFillControls`.
  - Headless-verified (`scripts/verify/gapFillsAndBorder.mjs`): border bbox grows by **exactly w/2** with the clip polygon unchanged (screenshot confirms the pattern reaches the border's inner edge untouched), and mixed rings render behind the reveal mask on **both** surfaces. Two script traps recorded in that README: identically-labelled rows exist on both substrates in Decoration (anchor to the border's, which come first), and `button:has-text("Clear")` also matches the panel's *disabled* "Clear paint" — a 30 s click timeout, not a wrong value. Tests **1712 green** (+10), tsc + build clean.

**Previous (2026-08-05, Strand line divisions 1–10 + line/gap ratio): SHIPPED + ✅ HEADLESS-VERIFIED (`362d370`).** User: *"increase max border stroke width and add toggles for the ratio between line and gap in the double, triple, dash and dot"*, then *"remove dotted and dashed lines and add line divisions up to 10"*.
...
**Previous (2026-08-05, Strands — "Apply to all Tiles"): SHIPPED + ✅ HEADLESS-VERIFIED.** User: *"a toggle for the strand editing features that is 'apply to all tiles' so if I change the angle of one tile, it changes it for the rest in the config."*
  ...
  - **Broadcast targets the LIVE Tile types**, not `Object.keys(config.figures)`: a loaded config's figures map can carry stale keys (`feedback_figures_map_pollution`), and fanning out to those would resurrect them.
  - **Line-set edits are id+kind gated.** Set ids are per-Figure (`set-1`, …) so they collide across independently-authored Tile types; `UPDATE`/`REMOVE_FIGURE_SET` only reach a Tile whose set of that id has the same `kind`. Figure actions are **not** on the history allowlist, so a mis-aimed broadcast delete would have had no undo.
  - **Read at dispatch time through a ref**, not callback deps: `config.figures` changes on every slider tick and would churn `dispatch`'s identity, and with it every child holding it.
  - UI: one checkbox at the head of the Strands section, shown only from two Tile types up, + a live "all N Tile types" hint. Reported in bug capture. Headless-verified on 4.8.8 (`scripts/verify/applyToAllTiles.mjs`): toggle **off** ⇒ Octagon 13° / Square 67.5° (diverged); toggle **on** ⇒ both read 75.5° off one drag. Tests **1692 green** (+12), tsc + build clean.

**Also 2026-08-05 (gradient stop cap 8 → 20): SHIPPED + ✅ HEADLESS-VERIFIED.** User: *"Increase the number of control points I can have in a gradient"*. One constant — `GRADIENT_MAX_STOPS` in `types/editor.ts` — and it was **UI-only**: `migrations.ts` validates a min of 2 and no maximum, so raising it invalidates nothing and a save authored at 16 already loaded fine. Stop-bar markers thin from 12 px to 8 px past 10 stops so they stay individually grabbable on a sidebar-width track, `+ Stop`'s tooltip names the cap, and the readout now leads with a live `n/CAP` count so the ceiling is visible before the button greys out. Shipped at 16, then **20 on the user's follow-up ("go 20")** — one constant either way. Verified in the browser (`scripts/verify/gradientStops.mjs`) on the across-frame surface: panel reaches **20/20**, `+ Stop` disables, and the rendered `<linearGradient>` def in the pattern SVG carries **20 `<stop>` elements** — the check is on the rendered defs, not the panel text, and the script reads the cap off the app's own readout so it needs no edit when the constant moves again. Spec updated (DECORATION_GRADIENTS_SPEC decision 2).

**Previous (2026-08-04, Decoration **Combine** — adjacent Voids act as one): SHIPPED + ✅ BROWSER-VERIFIED (`8e75aee` · `94b29f1` · `e2a3e1d` · `514aef5` · `384d4c8` · `9be114e` + seam-mask follow-up).** User: *"I want to be able to combine voids so they act as a single void in the decoration phase."* Confirmed scope: full Reach ladder (Matching included) and all four behaviours — flat fill + seamless render, gradients, stamps, hover/select as one.

  - **A Combine is a matcher, not an edit.** Voids are re-derived from the ray field every frame, so there is no Void to mutate between two frames; a Combine has to be a record that re-finds its own members, exactly like a `ColourRecord`. Stored as an **anchor** named at a Reach rung plus the other members' centroids **in the anchor's canonical pose** — the frame `stamps.ts` already uses to land one image consistently on every congruent Void. That is the whole trick behind the Matching rung: an offset relative to the anchor's own shape carries onto rotated and mirrored instances with no lattice or symmetry bookkeeping. `canonicalPoses` (new; `canonicalPose` is now its first element) exposes every tied pose, because a **symmetric** anchor needs all of them — only one aims the offsets at the real neighbours.
  - **A composite is just a bigger Void.** `applyVoidMerges` returns an ordinary `VoidRegion` with the union outline, so `colourVoids` / `resolveVoidStamps` / `buildVoidTargeting` / the paint overlay are untouched, and a composite is congruent to another composite of the same shape — `congruent` paint spreads across combined groups unchanged. One seam, five behaviours.
  - `polyUnion.ts` unions faces of one arrangement by edge cancellation. The non-obvious part is **re-splitting every edge at member vertices first**: `extractVoids` simplifies each face independently, so a T-junction kept on one side of a shared edge defeats naive pair cancellation and stitches the seam into the outline as a spike.
  - **Internal Rays disappear (`StrandSeamMask`).** First attempt repainted the group's own fill over its internal edges — which meant an *unpainted* combined group kept its dividing line, so a combine only took visible effect once you also filled it (user: *"When they are combined I want the internal strands to disappear."*). Now a `<mask>` on the strand layer, driven by `voidSeams` off the keyed field and deliberately independent of `voidFills`. Masking acts on rendered output, so curves, weave breaks, dashed/double/triple styles and per-strand colours all mask identically — cutting the seam intervals out of the `Segment` field would have had to reproduce every one of those AND would have re-chained the strands, moving the identities strand colours are keyed on. Band clipped to the union outline (can never reach a Strand outside the group), each quad overhanging its seam by a half-width (interior junctions close; outer overhang clips away). A/B verified: line across the waist gone with **no fill applied**, boundary strands untouched.
  - **Two real defects the work surfaced.** (1) `polyUnion`'s vertex grid was sized off the snap tolerance, making it quadratic in the pattern's *world scale* — 5.5 s to resolve 78 two-triangle groups on 3.6.3.6, on every paint; 11 ms sized off the mean edge length, now guarded by a scale-invariance ratio test. (2) A Combine disqualifies the periodic fast path (a group can straddle the fundamental domain), but `stampedField` wasn't keyed on `voidMerges`, so `fastPath` stayed stale-true and **nothing on the canvas changed** — the same trap the file already documents for the two gradient `enabled` flags. Only the browser found it; tests and tsc were green throughout.
  - Browser-verified on the 4.8.8 Builder preset: one Combine at Matching → **15 composites across the lattice**; one paint click filled all 15 as single shapes; a gradient ran continuously across each fused shape; Separate and Ctrl+Z both round-tripped. **Not** browser-verified: Stamps on a composite (needs a file upload) — covered by the probe suite asserting the clip equals the union area.

Tests **1665 green** (+23: `polyUnion.test.ts`, `voidMerge.test.ts`, `voidMergeProbe.test.ts`, plus reducer/migration/eligibility cases), tsc + build clean.

**Previous (2026-08-04, download filenames follow the config): SHIPPED (tests + tsc green, ⚠️ NOT browser-verified).** User: *"I want downloaded files to save with the name of the saved config, if there is one"*, then *"Any config with a designated name should download with that name. The saved name should take priority."* Every export was hardcoded `islamic-pattern.{svg,png,json}` — a folder of downloads was indistinguishable.

  - New `src/export/fileName.ts`. `sanitiseFileBase` strips filesystem-illegal characters (`/ \ : * ? " < > |`, control codes) and collapses whitespace to `-`, but **keeps dots** — the names that matter here are Configurations (`4.8.8`), and an over-eager slug would have eaten them. Leading/trailing `.`/`-` stripped (hidden files), capped 80 chars, and the cap re-runs the trailing-separator strip because a slice can re-expose one. Nothing usable left ⇒ `null`, so callers own their fallback.
  - `patternDisplayName(config)` is the second rung: a converted Patch names itself after `editor.presetId`'s `TILINGS` label, a legacy render after its own tiling label, and a Patch with neither after its Builder `configuration`. A from-scratch Patch designates nothing and returns `null` — the only case that still gets `islamic-pattern.*`.
  - Priority chain lives in one place, `exportActions.ts::downloadName` = `resolveSaveName?.() ?? patternDisplayName(config)`. Library name wins because it is the name the user typed for *this* piece of work.
  - **`resolveSaveName` is a getter, not a value, and that is deliberate.** `patternLibrary.get(id)` is `list().find(...)` — it parses and *validates every saved config* out of localStorage, so calling it per render would put the whole library on the render path. It also has to read the name **as of the click**: rename is handled inside `ConfigLibraryPanel` and does not necessarily re-render the Lab, so a memoised value could name the file after the old title.
  - `exportSVG` / `exportPNG` (via `PngExportOptions.name`) / `saveJSON` / `exportUnwovenSVG` all take the name; Gallery detail + card exports pass `save.name` / `entry.name` directly.

Left alone deliberately: the Decoration **stamp-canvas** downloads (`void-xxxx-canvas.svg`) and the Generator dataset export — those name a *shape template* and a *dataset*, not a config, and a stamp canvas is meant to be reused across saves. Tests **1621 green** (+12, `src/export/fileName.test.ts`), tsc clean. Commits `4241d39` + `536441b`.

**Previous (2026-08-04, Decoration-Phase Undo + Clear paint): SHIPPED + ✅ USER-CONFIRMED 2026-08-05 ("undo paint and clear is working").** User asked for "an undo and clear button in the decoration phase". Both half-existed, and finding out how they were broken *is* the ticket:
...
Tests **1609 green** (+11, `src/state/decorationHistory.test.ts`), tsc + build clean. ✅ **User-confirmed 2026-08-05** — nothing left here.
...
**Three decisions that are the actual content.**
  - **`ghostStampsOnly` exists because the two stamp sets disagree.** The Design "Show neighbours" set already excludes the centre copy; the **Composition Lattice set does not**. Without the filter, Composition double-strikes the live Guides with a ghost at the identity.
  - **Neighbour Anchors are derived from a Patch narrowed to the stamping Guides.** Reusing the full `collectGuideAnchors` would reproduce a crossing between a stamping and a **non**-stamping Guide — a point that exists only on the live Patch — on every neighbour. Tile-centre Anchors are dropped too (they belong to the Tiles, not a Guide) via a new `collectGuideAnchors(..., { includeTileCentres: false })` option.
  - **Pick semantics mirror neighbour *vertices*, not Guide Anchors.** A neighbour Anchor is **pickable but not grounding**, so `validateMultiPick` + `multiPickCompleteAcrossPatch` still refuse an Anchor-only neighbour polygon. They are always `stamp: true`, so a Complete there mints an ordinary repeating Cell Tile — no world-space `guideTiles` branch.

Render: the copies go in their own `#editor-guide-stamps` group **first**, faded + `pointerEvents: none`, with no hit stroke, drag handles or dashed extend span — a `both`-extended Guide repeated across 119 stamps would otherwise wash the canvas out. Overlay toggles (session state, none persisted): Design **Show guides** (Construct overrides it) + nested **Show anchors**, a **Show guides** under the neighbours group, Composition's existing toggle now covering the stamped copies + its own **Show anchors**. The three default-ON toggles report their **OFF** state in bug capture — listing them when on would drown the fact, and it is the off case that explains a missing overlay.

**Browser-verified headless**, each toggle exercised independently against DOM counts: stamp ON + neighbours ON ⇒ **119** ghost copies / 357 ghost Anchor dots; neighbour-guides OFF ⇒ 0 ghosts, live Guide untouched; anchors OFF ⇒ ghosts keep their 119 strokes but drop all 357 dots; Design "Show guides" OFF **in Place mode** ⇒ layer gone, and re-entering Construct brings it back (the documented override); Composition hidden by default, then 119 copies under the full Lattice. **Export exclusion proved structurally**: all 119 stamp groups sit under `data-export="exclude"`, and stripping that attribute's subtree removes both the guide colour `#9a5bd2` and the whole layer. Zero console errors. Tests **1598 green** (+19: `editor/guideStamps.test.ts` 16, `state/guides.test.ts` 2, `labBugFacts` 1); the two reducer/validator tests were **verified red** against the pre-#30 pick gate. tsc + build clean.

⚠️ Note for whoever automates the Lab next: `localStorage['lab-state-v1']` is written on a **debounce**, and drawing a Guide alone did not flush it within 1.5 s — the first verification script read `null` and looked like a broken draw. Read pick/geometry state out of the **SVG DOM** instead (that is what the final scripts do).

**Previous (2026-08-03, #29 — Guides slice 4, symmetry-orbit drawing + linked Guide groups): SHIPPED + ✅ BROWSER-VERIFIED (`ffd9a6b` engine/reducer, `e2295b6` UI).** User asked what models #29/#30 need, then said go. Drawing a Guide inside a Cell whose **Symmetry** picker is active now lays down the whole orbit at once — same lever as tile placement, no separate control (spec Decision 8).

**The frame problem is the whole ticket.** Guides live in **Patch-world** coords but `Sym` is a linear map about the **Cell-local** origin, so every transform round-trips `inverseCellTransform` → `applySym` → `applyCellTransform` (`editor/guideOrbit.ts::transformGuide`). Lines carry **both endpoints** through, which keeps parametric `manualAnchors` valid for free; circles carry centre **and** `guideCircleRadiusPoint`, so `phase` falls out without a rotation/reflection special case — and a reflection negates the circle's CCW `manualAnchors` (t → 1−t) since the rim's sense flips.

**Two decisions worth remembering.**
  - **Host Cell is strict containment, not `resolveHostCell`.** `guideHostCell` uses `cellContainingPoint` on the *anchor click* (line `start` / circle `center`). `resolveHostCell`'s nearest-Cell fallback (right for #34's Tiles) would hand every canvas-space Guide a host and make the spec's "Guides drawn outside any Cell always draw as singles" **unreachable**.
  - **The symmetry mode is snapshotted, not read live.** `GuideGroupRef` = group id + `cellId` + `mode` + this member's `symIndex`. Re-reading the Cell's picker would mean flipping it to `'none'` and nudging one endpoint silently deleted the other 11 members. `symIndex` is what keeps ids stable across an edit (React keys, the open popup, and the undo coalesce key all key on id).

**Regeneration keeps the orbit closed.** `regenerateGuideGroup` treats the edited member as authoritative: pull back through `inverseSym(own)` to the group's base frame, push out through each sibling's own element. Settings copy verbatim; geometry reshapes the orbit *symmetrically* rather than stacking every member on the dragged one. Group closure is why no inverse-matching by position is needed — applying the whole subgroup to any member reproduces the same orbit set. Fails closed (returns the input array) when the host Cell is gone.

Reducer: ADD expands, UPDATE regenerates, DELETE removes `guideGroupIds` — **one action each**, so undo treats a group operation as one step (the ticket's requirement, satisfied by construction, not by new history code). `group` is typed out of `EditorGuidePatch` **and** stripped at runtime by `mergeGuide`; membership changes only by drawing or deleting. Migration validates the ref and drops a malformed one to an unlinked single rather than regenerating siblings off garbage. Divided circles never expand (self-symmetric via their n-division).

UI: `EditorGuideLayer` lights every member when one is selected (handles stay on the clicked member — 12 handle sets would bury the artwork), `GuidePopupOverlay` badges "⇄ linked ×n" and re-labels delete "Delete all n", `DesignPanel`'s Construct help says what Symmetry will do.

**Browser-verified headless** (fresh Patch → Symmetry → Construct → two-click draw): D4 draws **8** members from one line and renders as a rosette scaffold; rotation-only draws 4; dragging one endpoint moves **all 4** ends to **4 distinct** positions (reshaped, not collapsed); the stamp checkbox flips all 4; "Delete all 4" clears the block. Zero console errors. Tests **1579 green** (baseline 1554; +19 `editor/guideOrbit.test.ts`, +6 in `state/guides.test.ts`), tsc + build clean.

⚠️ One test expectation was wrong before it was right: the shared `guide()` helper in `state/guides.test.ts` runs along the **horizontal mirror axis**, whose D4 orbit legitimately collapses 8 images → 4. The group test now draws a generic off-axis line; the collapse itself is covered separately.

**Previous (2026-08-03, #40 — vertex-line rays leaking past their own polygon): SHIPPED + CLOSED (`98c60a0`).** User asked what's next, picked #40 off the recommendation (already scoped by the earlier backlog pass, no decision needed). Two independent leak mechanisms in `emitVertexArms`: (1) the reported case, α = 90°−θ exceeding the vertex's interior half-angle, so the ray direction points outside the polygon's own tangent cone and `clipSegmentToPolygon` returns the unclipped natural endpoint — fixed via an angular-betweenness test (`vertexRayEntersPolygon`), *not* a spatial `pointInPolygon` probe: the probe was tried first and broke `figureSymmetry.test.ts` (C1-of-C3 breaks at exactly the boundary, the identical instability #51 hit on the edge-line pairing), so it was replaced before landing. (2) A second leak, found only because the fix was verified by widening the morph probe's sweep past its old safe band: an asymmetric ray pairing (one ray's shared-intersection `t` negative) reused that point as its own natural target even though it sits behind its own origin — reachable only under a per-vertex Morph field (different effective θ at each end of a pairing). `vertexStrandsOverlap.test.ts`'s contact angle moved 30°→60° on a square (30° now legitimately emits nothing, the "some counts will drop, correctly" the backlog pass anticipated). New regression suite `src/pic/vertexRayLeak.test.ts`, verified red pre-fix. Tests **1554 green** (baseline 1551), tsc + build clean. Issue #40 closed with the mechanism writeup.

**Previous (2026-08-03, #31 — Anchor vocabulary consolidation): SHIPPED + CLOSED (`10382c3`).** User asked for the next zero-input task; #31 was the cleanest — its blocker #28 was already shipped. CONTEXT.md's Construct/Guide/Anchor entries turned out to already exist from ADR-0008 (written alongside the guides work); the real gaps were (a) the Anchor entry still pointed at #33 as an unshipped follow-up when both #33 and #34 have since landed, and (b) actual UI copy still said "vertex" for Anchor pick targets. Folded #33/#34 into the CONTEXT.md Anchor paragraph, then swept the copy: `EditorPickerOverlay.tsx` picker header ("Anchor at Vertex" → "Place at Anchor"), hint text ("at this vertex" → "at this anchor"), aria-label; `DesignPanel.tsx` Complete-mode multi-pick prompts ("N vertices selected" → "N Anchors", "outer vertex/vertices" → "outer Anchor(s)"); `patchSelectable.ts` validity labels ("Pick at least 3 vertices" → "Anchors", "each vertex must be distinct" → "each Anchor must be distinct"). `CLAUDE.md` vocabulary quick-map gained a Construct/Guide/Anchor line. Left alone: code identifiers/comments (policy — legacy names ok in code), and the PIC/Figure sense of "vertex" (`vertexContactAngle`, "Vertex strands", a Tile's own geometric vertex in prose) — that noun is unrelated to the Place/Complete pick-target umbrella and stays as-is. Tests **1551 green**, tsc clean. Issue #31 closed with the summary. Open Guides-label issues now: #29, #30, #32 (32 blocked on 30).

**Previous (2026-08-03, zero-input backlog pass — issue hygiene + #40 scoping + #34): SHIPPED (`a8af5b2`), ⏳ browser-verify the #34 half.** User asked what could move without them, then said go. Three things landed.

**(a) Issue hygiene — 5 issues closed**, each with a pointer to its verifying commit: **#44** (gradients slice 1, verified in the 2026-07-22 sweep `fd56485`), **#45** (across-frame, `cfccc02`), **#46** (Strand gradients, `7795c85`/`009f3f7`/`1cca660`), **#42** (multi line sets, sweep item 1 — noted on the issue that `296f9ac` still owes a browser pass), **#28** (Guides slice 3, user hand-confirmed 2026-07-22).

**(b) #40 scoped with a throwaway probe** (written, run, deleted — full findings on the issue). Every shipping tiling × θ ∈ {20…80}, counting vertex lines and testing each endpoint for containment in its own tile. **The predictor is exact for detection: α = 90°−θ > the vertex's interior half-angle ⇒ leak, 88 cases, 88 predicted, zero unpredicted.** 21 predicted-but-clean, all tiles whose arms were already fully suppressed. **20 of 26 tilings** affected. Two corrections to what we believed:
  - ⚠️ **"#40 is the only remaining reason a tile emits fewer than 2n" is FALSE.** The probe separates two regimes: 88 **leaks** (full 2n emitted, endpoints *outside* — the bug) and 28 **dropouts** (fewer than 2n, nothing outside). Every dropout sits at α ≈ the half-angle *exactly* (square@45, hexagonal@30, triangular@60) — the ray runs along the edge, degenerate, correctly dropped. #40's own signature is the opposite: right count, wrong geometry. Fixing it will push some counts *down*, correctly.
  - **It is not a low-θ problem.** `pentagonal-rosette`'s 18°-half-angle quad leaks at **θ=71°**, past the classic 67.5°; `tetrakis-square`'s 22.5° triangle at θ=60.
  - **The issue's "clamp the slider per shape" option is dead.** Partial leaks are common on irregular tiles (`decagonal-rosette` 6-gon @40: 12/12 lines, 8 of 24 endpoints out) — one θ is simultaneously safe at one vertex and leaking at another, so no per-shape safe θ exists. The per-arm clip is the only route, and it must suppress the individual **arm**, not the tile's set.

**(c) #34 FIXED + CLOSED (`a8af5b2`).** `placeTileOnGuideAnchor` filed a stamping-Anchor Tile in `activeCell` — "last Cell mutated", not user intent. Now resolves by containment via **`resolveHostCell`** (the helper the ticket specced before it existed; `fa4e89d` built it for Complete). Went wider than the ticket in two necessary places: **sizing** follows the host too (`cellPlacementEdgeLength`), and **`Canvas.anchorEdgeLength`** had to move with it or preview and commit would disagree. The orbit re-target is the substance — `localAnchor` / `localRotation` / `boundarySymmetries` / per-image transforms / `updateCell` all run against the host. ⚠️ **The ticket's "rendering is unaffected" held only with symmetry OFF**: on the 4.8.8 seed an Anchor inside the *square* Cell hosted on the *octagon* orbits about the **origin**, flinging images to (±198, ±198) across sibling Cells — that is now the load-bearing test. 5 new tests, **each verified red** against the old hosting. One existing test changed: the multi-cell sizing test asserted *hosting* while testing *sizing* (its `FAR` anchor is outside every Boundary, so the nearest-centre fallback now picks the square, not the active octagon) — retargeted, property unchanged. Suite **1551 green** (baseline 1546), tsc + build clean. ⏳ **Browser-verify:** 4.8.8 Patch, Design → Construct a Guide with `stamp` on crossing the square Cell, → Place on one of its Anchors inside the square, square Cell symmetry **Full** ⇒ images stay in/around the square, not scattered across the Patch.

**Previous (2026-08-03, Void seams + a canvas background control): SHIPPED, ✅ USER-CONFIRMED** ("they're all working", covering this and the Frame-origin fix below). User: *"With strand colour off there are still very small unpainted pixels between voids. Improve the void geometry and add background painting option (or tell me where it is if I missed it)."*

**Not the geometry — antialiasing.** Two Voids sharing an edge each cover ~half of every pixel along it, and two half-covered composites don't reach full coverage, so the background shows through as a hairline tracing the whole strand skeleton. Worst exactly where it's least wanted: "Remove strand colour" + one flat colour, which the panel's own comment promises will "meet seamlessly". **Measured, not argued** — a central sample block of a flat-painted floret-pentagonal field: **2.95%** part-covered pixels, and **zero** fully-background pixels (a real gap would show the paper tone at full strength, not a blend). Fix: stroke each fill with its own paint (a gradient `url(...)` strokes fine) at `vector-effect="non-scaling-stroke"`, width 1 — the artefact is screen-space so the correction is too, leaving geometry untouched at every zoom and giving a raster export at any resolution the same one-pixel overlap. Result **2.95% → 0.001%**. Confirmed on the **Lever-A `<use>` fast path** as well (`non-scaling-stroke` survives cloning): 4.8.8 **2.619% → 0.65%**, the residual being the Cell-Boundary + tile outline overlay, which is `data-export="exclude"` scaffolding, not seam.

**The background control did not exist** — nowhere in the app, on any substrate. `strand.background` has always been the canvas backdrop and the far stop gradients seed from, but it could only ever arrive from a preset. Now a **Canvas background** row in the Decoration panel (swatch + "Use paint colour" + Reset), dispatching `SET_STRAND_STYLE`. Deliberately **not** a new `DecorationConfig` field: one backdrop per pattern, already persisted, already in `PATTERN_CONFIG_KEYS`. Third piece, or the feature would be a lie: **PNG export flattened onto a hard-coded `#f5f0e8`** — it now passes `config.strand.background`, which is what the `DEFAULT_PNG_BACKGROUND` comment ("a future Decoration background-colour option will replace this default") had been waiting for. `DEFAULT_CANVAS_BACKGROUND` in `state/defaults.ts` is now the single literal.

⚠️ **Two knowingly left open.** (a) **Vector SVG export** carries the background only as the root `<svg style="background:…">` — browsers honour it, Illustrator/Inkscape generally don't. A real `<rect>` underlay would change every existing export, so it stays on the parked export list. (b) **Void stamps** still abut (a `<clipPath>` can't be stroked); their seam shows the fill beneath rather than the paper, so it reads far weaker, but a stamped *unfilled* Void will still hairline.

**Previous (2026-08-03, legacy Frame seeds on the view): FIXED, ✅ USER-CONFIRMED 2026-08-03.** Bug capture: *"stamp and void fill removed by frame"* — legacy substrate, floret-pentagonal, Decoration, `frame: shape/square/400/clip`, 3 Void fills + 1 Stamp still in the config. **Not a decoration bug at all.** The filed config renders perfectly: fills and stamps sit INSIDE the frame-clipped group (`PatternSVG.tsx:309-310`), and on this substrate the Frame touches nothing upstream of render — the Void extraction bound is the quantised **viewport**, never the frame outline (`usePattern.ts:831`), and `SET_GALLERY_FRAME` only writes `state.frame`. Four headless scenarios (config as filed ± frame, paint-then-frame, frame-then-paint, Export SVG) all kept 65 fill paths and 15 stamp images.

The trigger was the one the previous entry had already named and left unbuilt: **a legacy Frame was created at world (0, 0) regardless of where the camera was.** The camera is free and isn't persisted, so creating a Frame while panned away clips the entire visible field out — canvas goes blank, and a reload (which recentres) makes it "come back", which is exactly what the user reported. Reproduced: pan to viewBox `1636 1103 …`, click **+ Shape Frame**, clip polygon lands at ±283 around the origin ⇒ nothing on screen survives.

Fix: `FramePanel` takes `newFrameOrigin?: () => Vec2 | null` — where a **newly created / reset / ⊙-recentred** Shape Frame is centred. The Patch omits it ((0,0) = seed Patch centre, meaningful and always on screen); `LegacySubstrateControls` passes the live view centre off the `viewBoundsRef` it already held, read at click time so panning doesn't re-render the sidebar. Second half, needed to keep the fix honest: the origin X/Y sliders were absolute ±800, so a view-centred Frame thousands of units out would snap back inside range on first drag — the span now grows to cover the Frame's own origin, **monotonically** (a shrinking span would move the track under the thumb mid-gesture). `readGalleryFrame` already round-trips `origin`, so nothing new to persist. Tests **1546 green**, tsc clean.

**Previous (2026-08-03, Frame on view-only presets): SHIPPED, ✅ BROWSER-VERIFIED (headless).** User ask: *"I want to be able to frame the patterns that are 'view only' in the lab."* It turned out to be a **pure UI gap, not a feature**: `Canvas` already clipped a non-editor tiling to the top-level `config.frame` and already drew its border stroke (`Canvas.tsx:313-337`, `clipToFrame = config.tiling.type !== 'editor' || editorStrandMode`), `SET_GALLERY_FRAME` already existed in the reducer, and `frame` was already in `PATTERN_CONFIG_KEYS` — but **nothing in the app could create one**. `SET_GALLERY_FRAME` had exactly one call site: DecorationPanel's border-stroke edit, gated on `{frame && …}`, i.e. only reachable for a frame that already existed. So legacy Frames could only arrive from an old save.

`FramePanel` is now **substrate-agnostic** — props `substrate: 'patch' | 'legacy'`, `frame`, `onSetFrame(f | null)`, `nRingSupported` — instead of reading `editor` + dispatching `SET_FRAME` itself. The Builder passes `editor.frame` + `SET_FRAME`; `LegacySubstrateControls` passes `config.frame` + `SET_GALLERY_FRAME`. Legacy hides the **Frame type** row and the **Clear frame tiles** button (no Patch ⇒ no completion, no lattice to ring), creates Shape frames with `boundaryTreatment: 'clip'`, and gets its own note + origin tooltip. Everything else is shared. The old inline n-ring predicate moved to `nRingFrameSupported(patch)` in `editor/frameNRing.ts`.

~~**Offered, awaiting the user's word:** a legacy Frame is seeded centred on the **world origin** at half-extent 400 … seeding the Frame from the **current view** instead of (0,0) is the alternative and was offered, not built.~~ → **BUILT the same day** (see the latest entry). It was not only an aesthetic default: the world-origin seed could put the whole Frame off screen and blank the canvas.

Verified in headless Chromium against the dev server, both substrates: Cairo Pentagonal (view-only) → "+ Shape Frame" present, "+ n-Ring" absent, canvas clip polygon appears, shape→hexagon and size both change the clip polygon, Frame-border-stroke slot reachable in Decoration, "Remove Frame" drops the clip; 4.8.8 (Patch) still offers both types and the n-ring clip still builds in Composition. Tests **1546 green**, tsc clean.

**Previous (2026-08-02, Mirror — third mode + Focus flip): SHIPPED (`de5e4d8`), ⏳ browser-verify.** `4a0d2dd` made Upright *correct* where the shape allows, and the user still reported **"doesn't look any different in the pattern view"** — then asked for the blunt instrument instead: a third setting that reflects all the other way, plus a flip in Focus mode. Right call, and taken as given. **`mirror: 'all'`** corrects the *complement* half of `'never'`, so the class agrees on the opposite handedness — three rungs, **Reflect | Upright | Flipped** (row control cycles the same three). **`StampUserTransform.flip`** mirrors the image about the canonical box's vertical centreline, composed **innermost** so the rotation slider keeps reading the way the user sees it; exposed as "⇄ Flip" in Focus mode. Between the two, every orientation of the artwork is now reachable directly, with no inference about what the geometry "should" do. Both fields optional + additive. Tests **1546 green**, tsc + build clean.

⚠️ **Unresolved: why `4a0d2dd` showed no visible change on the user's pattern.** Either their Void class is chiral (fallback path — plausible, the screenshot looks rosette-ish) or `canonicalSelfMirror` isn't firing there. The new controls make it moot for authoring, but the question is still open, and nothing in this session has been verified in a browser. **A visual harness is the missing tool** — three sessions running now have turned on "reasoned correctly, looked wrong", and each round-trip cost the user a screenshot.

**Previous (2026-08-02, Upright vs the Focus layout): FIXED (`4a0d2dd`), ⏳ browser-verify.** User, after the first Mirror cut: *"With mirroring off, the stamp renders in the mirror opposite to the one shown in focus mode. With mirroring on, some are the same and some aren't… I can never achieve all in the orientation that I have chosen in focus mode."* Correct, and the first cut caused half of it. Cancelling the pose's reflection by mirroring the **image** about the canonical box centreline fixes the *handedness* — but that centreline is almost never an axis of the shape, so the layout lands mirrored **against the outline it was chosen against**. Handedness right, placement wrong; the user reads the composite, not the determinant. (My test asserted `det > 0` and was happy — it measured the half of the property I had thought about.)

Fix: **`canonicalSelfMirror(points)`** — a reflective self-symmetry of the canonical outline. Orientation-reversing ⇒ it reverses vertex order, so the candidates are the `n` maps `points[i] → points[(s−i) mod n]`, each built from the first edge's image and verified pointwise (O(n²), memoised per signature). Composed as **`toInstance ∘ M`, OUTSIDE the user layout**, the pair is a pure rotation+translation: motif upright AND the Focus placement arrives rigidly moved. **Search the IDENTITY outline, not the flattened one** — a flattened curve's sampling isn't symmetric, and that one change took `4.8.8` @0.3 from **4 → 60 of 84** affected instances (`3.6.3.6` 84/96). A genuinely **chiral** Void has no `M` and *cannot* have both properties — `decagonal-rosette` is mostly chiral (19/152) — so it keeps the box-mirror fallback and the panel now states which property Upright gives up instead of leaving the user hunting for a setting that can't exist. Tests **1542 green**, tsc + build clean.

⚠️ **Not done, deliberately:** making `canonicalPose`'s tie-break prefer the unreflected traversal when the tied set has one. It would make the DEFAULT stop mirroring gratuitously on symmetric shapes — but the pose is shared with gradients and #54, so it would move existing saved work. The toggle gets the same result opt-in.

**Previous (2026-08-02, "stamps are flipped"): EXPLAINED + toggle SHIPPED, ⏳ browser-verify.** User screenshot: a directional leaf motif reading **backwards on about half the Voids**. Not a glitch — **structural, and measured**: `voidSignature` → `minRotation` minimises over the token ring **AND its reversal** (`voids.ts:680`), so a shape and its mirror image are **ONE congruent class**, and `canonicalPose` honours that by *reflecting* the pose on the opposite-handed half (`flip = -1`; the doc already said "rigid(-**or-reflected**)"). Probed on real fields: `decagonal-rosette` splits every populous class **exactly 5+/5−**, `4.8.8`'s largest **20+/20−**, `3.6.3.6` **18+/18−**. Half of every class, every field.

Correct for abstract artwork (the stamps inherit the tiling's own reflection symmetry) and wrong for a motif with a direction — so it is a **per-record choice, not a fix**: `VoidStampRecord.mirror?: 'never'` (absent = `'reflect'`, the original behaviour, so every existing save is untouched). `'never'` composes `mirrorXMatrix(box)` **innermost** — the image is pre-mirrored in canonical space and the reflected pose mirrors it back, so the motif lands upright while the pose, the fit box and the clip are all untouched. Panel: **Mirror · Reflect | Upright** under Overlap, plus a `⇄` toggle on each stack row. `isReflectedPose` (det < 0) is the shared predicate. Pinned, not asserted away: `stamps.test.ts` now carries the 50/50 handedness split as a characterization test, so a future change to the tie-break has to face it. Tests **1540 green** (baseline 1536, +4), tsc + build clean.

**Previous (2026-08-02, stamp Overlap + stacking order): SHIPPED, ⏳ browser-verify.** User ask: *"when using stamps I want to be able to toggle to allow stamped images to overlap and add a small UI to bring forward / send back certain tiles."* Two halves, and the second only means anything because of the first. (1) **Overlap** — `VoidStampRecord.overlap?: boolean` (optional + additive, `true` only; turning it off deletes the field so pre-Overlap saves stay byte-identical). On ⇒ `VoidStampLayer` drops the `clipPath` and the image draws whole, spilling onto its neighbours; pair it with Focus-mode zoom, which is what pushes the image past the shape's box. The pose and the cover/contain fit are **unchanged** — Overlap only removes the clip, so toggling it can never move a stamp. (2) **Stacking order = array order** in `decoration.voidStamps` (last paints last ⇒ front). `resolveVoidStamps` now emits **record-major** (all Voids of record 0, then record 1, …) instead of void-major, so the array actually is the paint order; new `REORDER_DECORATION_VOID_STAMP` (`forward`/`backward`/`front`/`back`, clamped, no-op returns the identical state) drives ▲/▼ in the panel's **"Stamp stack — front to back"** list, rendered top-of-stack down like any layer stack.

Two traps worth naming. **`SET_DECORATION_VOID_STAMP` used to `filter` + `push`**, so re-uploading an image moved that record to the front — invisible before, order-destroying now; it upserts **in place**. And an Overlap stamp is **ineligible for the Lever-A fast path**: inside the `<use>`-tiled fragment each clone paints whole in order, so a spill near a domain edge would land *under* the next clone's tiles instead of over its true neighbours, and the cross-record order would stop holding. Same treatment as `frameGradient` / `strandGradient` (`periodicFastPathEligible`). Focus mode previews an Overlap stamp unclipped at full opacity with the margin growing with zoom (capped), since nothing is being cropped away. Tests **1536 green** (baseline 1530, +6: resolver order + flag pass-through, reducer in-place upsert + reorder + end/unknown-key identity, migration round-trip), tsc + build clean.

**Latest (2026-08-02, the 1.5° curve cliff): FIXED (`a97e079`) and ✅ BROWSER-VERIFIED** ("looks all good to me"). User picked it off the NEXT menu. `extractVoids` simplified every outline at a fixed **1.5°** collinear tolerance — right for the identity outline (it stops a T-junction vertex splitting a congruent class) but the same pass produced the outline that gets **drawn**, and a curve of control offset `d` turns only ~`1.5°·d/0.0525` per flattening chord. Below that the curve was discarded wholesale: 4.8.8, offset **≤ 0.05 ⇒ 0/59** un-clipped Voids curved, **≥ 0.055 ⇒ 59/59**. Void fills, exported stamp canvases and both Focus editors went straight-edged while the strands beside them still rendered bowed. **New finding — on `3.6.3.6` it isn't even a clean cliff:** at offset 0.01 the shorter triangle edges flattened while the hexagon edges didn't, so a single field rendered **90 Voids curved and 13 flat**. Fix: `ExtractVoidsOptions.simplifyAngleTol` + `RENDER_SIMPLIFY_ANGLE_TOL` (0.05°, 30× tighter), passed by `extractDecorationVoids` on the **curved pass only**. That pass is render-only *by construction* — `pairCurvedOutlines` takes both `keyPolygon` and `signature` from the straight pass — so nothing here can move identity; **asserted, not argued** (signatures / key outlines / areas byte-identical to the strict extraction, while the drawn outlines differ). **No new cost regime:** point counts below the cliff land on exactly the values the app already pays above it (4.8.8 avg 7.6 → 60.7, the offset ≥ 0.055 figure) and extraction time is unchanged. `curvedFieldFixture` now mirrors `extractDecorationVoids` including the tolerance — a fixture that simplified harder than production would hide the difference its tests exist to measure. Verified red-first (all 5 fail against the old behaviour). Tests **1530 green** (baseline 1525), tsc + build clean. Suite: `src/decoration/voidCurveFidelity.test.ts`.
...
**The user-visible half was a SECOND defect, found only because they tested it (`4ba619d`).** Report: *"Void shape in focus mode isn't curved."* The periodic fast path rebuilt each `PaintVoid` for the Paint overlay from a rep and **dropped `keyPolygon`** — so every consumer of a Void's *shape* (stamp canvas, BOTH Focus editors, gradient extent) saw identity === rendered, posed off the flattened curve, and laid the shape out in a frame **rotated away from the renderer's** (4.8.8 @0.3: a 121×98 canonical box became 102×120). Dropped in exactly one object literal; `instanceVoidFills` 26 lines above already translated it correctly. **Affects every Builder patch** — the fast path is what serves them — so it also explains the stamp Focus editor and takes `2d4e504` off the unverified list. Tests **1525 green** (baseline 1514), tsc + build clean.

⚠️ **Lesson worth keeping: source-level correctness was not enough.** Both Focus editors read `stampGeometry` correctly the whole time; the bug was in the data handed to them, one hop upstream. Reading the consumers "proved" it was fixed. Only the user's browser pass found it.

⚠️ **One unexplained flake.** A single full-suite run reported **2 tests failing across 2 files**; seven subsequent runs (4 parallel, 2 `--no-file-parallelism`, 1 post-build) were all 1525/1525 green and the failing tests were never identified. Timeout is 30 s against a 3.3 s slowest test, so load does not explain it. If it recurs, capture the file names.

**⚠️ Found en route — #54, and it is NOT gradients-only.** Congruent instances on a curved field can pose through canonical frames whose *rendered* boxes differ ~9% (same area, same perimeter, same shape; straight boxes identical). It sits in `canonicalPose`'s tie-break and hits **stamps identically** — confirmed: one signature, 40 instances, **two different fitted image sizes** (121.32 vs 111.52). That is a live defect in a shipped, browser-verified feature. Pinned by test, not asserted away; fixing it touches Void identity, so it needs its own call.

**Three things deliberately NOT changed — (c) has since been FIXED (`a97e079`, above).** (a) `bboxAxisAtAngle` stops the axis where it **exits** the box, so at oblique angles a corner of a non-square box projects past the end and renders flat — independent of curves, pinned by an existing test that names the behaviour, and widening it would change every Fit plus the frame/strand washes. Pinned as known in the new suite. (b) The legacy substrate's view-rect seed fallback is documented and deliberate (an infinite field has no content extent), not a bug.

(c) ✅ **FIXED 2026-08-02 (`a97e079`) — was: gentle curves discarded from Void outlines entirely, at a sharp cliff.** `extractVoids` runs `simplifyCollinear` at a fixed **1.5°** tolerance (`voids.ts:364`) on the extracted outline. Measured on 4.8.8: **offset ≤ 0.05 ⇒ 0/59 Voids keep the curve; ≥ 0.055 ⇒ 59/59 do.** Below the cliff the strands still render bowed while the Void fill, its exported canvas and Focus mode all go straight-edged. The 1.5° simplification is *correct for the identity outline* (it stops a T-junction vertex splitting a congruent class) and wrong for the rendered one. Fix = simplify only the identity outline, leave `polygon` at full fidelity — **done exactly that way**, see the latest entry. Not the cause of the reported bug (that was `4ba619d`), found while diagnosing it.

**Previous (2026-08-02, in-app Bug capture): SHIPPED (`c2fc22f`, `f877ab2`) and ✅ BROWSER-VERIFIED.** User asked for "a space to log a bug note that automatically captures relevant information from whatever screen and function I'm using — pattern configuration details, screenshots etc." Delivered: top-bar bug button + **Ctrl/Cmd+Shift+B**; the panel captures on **open** (not on save) — the active screen's own facts, the verbatim `PatternConfig` + a readable digest, a PNG of the live canvas, and the last 50 console errors/warnings. Out again as Markdown (clipboard or file), a self-contained JSON bundle, or the PNG. Reports persist in their **own** IndexedDB db. Architecture note in CLAUDE.md; module is `src/bugreport/` + `components/BugReportPanel.tsx`.

**Durability follow-up (`11ad447`), ✅ verified.** Prompted by "under what circumstances might these records be lost?". (a) A refused store write said *"Saved to this session only"* — false; nothing retained the report. Now it states total loss and holds the report behind a rescue banner (Download JSON / .md / clipboard). `saveOutcomeMessage` is pure, pinned by a test **verified red** against the old string. (b) `requestPersistentStorage()` on first save — origin-wide, so it covers the pattern library too; **a mitigation, not a guarantee** — confirmed *denied* on localhost headless Chrome. (c) **Dev server pinned to 5173 with `strictPort`** — storage is keyed to origin *including port*, so a silent hop to 5174 presents an empty app (no patterns, thumbnails, dataset or reports) with the data unreachable until you return to that port. ⚠️ **If an expected report is missing, it was filed on another port** — `npx vite --port 5174` once to reach it.

**Key design call:** screens *contribute* their context (`useBugScreenContext`) rather than the panel reaching in — prop-drilling a dozen values out of the 1100-line `TessellationLabMode` to feed a modal would have been worse than the bug reports. Contributors sit in a ref keyed by `useId`, so registering costs no re-renders.

The browser pass earned its keep — **three defects only it could find**: the panel sat under `.top-bar` (z-index 120 vs 150) so the top bar painted over its header; the scroll area kept flexbox's default `min-height: auto` and overran the dialog's own `maxHeight`, clipping it top and bottom; and the Lab reported `Substrate: legacy` on an empty Lab while the Pattern digest said `empty` — two contradictory answers in one report, because that flag is a decoration-scope binary (now labelled "Decoration substrate", Decoration Phase only). Tests **1512 green** (baseline 1466, +46), tsc + build clean.

**Previous (2026-07-31, symmetry Complete places copies in neighbouring Cells): FIXED (`fa4e89d`), ⏳ browser-verify.** User report + screenshot: in **4.6.12**, Completing a gap with symmetry on scattered Tiles across the neighbouring Cells. Cause: `multiPickCompleteAcrossPatch` hosted the Tile in `activeCellId` — which stopped being a *user* selection in 2026-06-18 and is now just "the last Cell mutated", and **turning symmetry on for a Cell is itself such a mutation**. So the natural workflow (enable symmetry on the dodecagon → Complete a gap in a square Cell) ran the orbit in the dodecagon's frame under D12: 6 copies, two landing on square-2/square-3's seeds, three outside the Patch and therefore repeated at every lattice stamp. Fix: `resolveHostCell` (new, `patchSelectable.ts`) — Boundary containment on the pick centroid, nearest-Cell-centre fallback outside every Boundary. Rendering was identical while symmetry was off (lattice stamps are uniform translations), which is why the misfiling hid for so long. **#34 is the same assumption on the Anchor path and now has its helper.** Tests **1466 green** (baseline 1461, +5), tsc clean. Repro: `src/editor/completeHostCell.test.ts`.

**Previous (2026-07-31, stamp canvases straight-edged on a curved field): FIXED (`2d4e504`), ⏳ browser-verify.** Every stamp geometry path (export canvas, panel inspector, Focus mode, image-fit box) read the STRAIGHT `keyPolygon` while the stamp is clipped to the CURVED `polygon` — so the design guide was the wrong silhouette (a `3.6.3.6` triangle's curve encloses 66% of it) and `cover` fitted to a box the shape bulged 8.8% outside. Fix: one seam, `stampGeometry` — pose stays on the identity outline (curved outlines can't rank reliably under `canonicalPose`'s quantisation), shape + box follow the rendered one. Tests **1461 green**. Dated entry below.
...
- **A regression test can pass against broken code.** A cap that reached the tile centre via floating point read as `2.0e-14` and sailed through `toBeGreaterThan(0)`. Use size-relative thresholds, never compare a computed geometric quantity to exact 0, always verify red-first. **Hit again 2026-07-31 in a new form:** a stamp-canvas enclosure test compared the canvas points to *their own* bbox — a tautology that passed against the broken build. Red-first caught it. When asserting "A fits inside B", derive A independently of B.
- **A shared seam only holds if the second feature actually calls it.** Stamps got `stampGeometry` in `2d4e504`; gradients kept their own `canonicalPose` + `poseBBox` pair two files away and silently missed the fix — the identity-vs-geometry note below was already written and still didn't reach them. When a fix establishes a seam, grep for the *other* callers of what it replaced (`88bd29d`, #54).
- **An outline chosen for stable IDENTITY must not silently become the GEOMETRY.** Voids carry a straight `keyPolygon` (identity, curve-insensitive) and a curved `polygon` (drawn). Using the identity one for canvases, boxes and previews shipped a straight-edged guide for a curved shape. Ask per call site: does this want the thing's *name* or its *shape*? Memory: `feedback_identity_vs_geometry_outlines.md`.
- **`activeCellId` is not a user intent.** It has been an internal "last Cell mutated" pointer since the active-Cell selector was deleted (2026-06-18). Any Builder path that resolves a *target* from it is guessing — and per-Cell control edits (symmetry mode, boundary size, No Seed) silently move it. Prefer geometric resolution (`resolveHostCell`). Remaining readers to audit: `applyWrap`, the n-ring Frame, `patchSelectable`, `chordCompleteAcrossPatch`'s cross-stamp branch, `placeTileOnGuideAnchor` (#34).
...
**~~⚠️ NEW user report 2026-08-02 → GitHub #55~~ — ✅ FIXED 2026-08-03 (`6ea2ba7`), issue closed. Trace below kept because every claim in it held up.** *"I can't edit or add a frame to any of the presets loaded from the Gallery."* **Confirmed by source trace — a missing UI, not a broken one.** `FramePanel` is the only thing that CREATES a Frame or edits its geometry, it takes a Builder `editor` and dispatches `SET_FRAME`, and it mounts only from `EditorDesignControls` — so it needs a Patch and the Design Phase. A Gallery preset opened as a **legacy substrate** renders `LegacySubstrateControls`, which deliberately has **no Design Phase**. The confirming detail: `SET_GALLERY_FRAME` (the only writer of `PatternConfig.frame`) has **exactly one author in the whole app** — `DecorationPanel.tsx:306`, gated on `frame &&` and spreading `{ ...frame, stroke: s }`. So the Gallery Frame can be *styled*, never *created*, and its geometry is unreachable. Two load paths differ: the **Lab Presets shelf** (tier-1 conversion → a Patch → FramePanel works) vs the **Gallery browser / "Edit in Lab"** (legacy → nothing). ⚠️ **Ask the user which path they used** — that alone narrows it. Likely a gap opened by the convergence flip that made the Gallery read-only. Options + a Sonnet recommendation are on the issue.

**Latest (2026-08-05, working through the open items): 3 shipped + 1 corrected + a verification pass.**

  - **SVG exports carry the canvas background as a real `<rect>` (`63942ed`).** A CSS `background` on the root `<svg>` is presentation, not content — browsers honour it, Illustrator/Inkscape don't — so once the backdrop became user-editable an SVG export silently dropped a deliberate choice while PNG kept it. `insertBackgroundRect` is a pure markup transform sized off the export's **own** viewBox, so a Max-fill viewBox with a non-zero origin is covered (verified: `-130.07 -130.07 330.86 330.86`). The **Transparent background** toggle now governs both formats (`pngTransparent` → `transparentBackground`). Also fixed the **Gallery detail view**, which had the same drop in both directions — its PNG ignored the save's background and flattened onto the sandy default. ✅ Headless-verified: rect present + first-drawn on a 550-`<use>` Composition field, absent with the toggle on, correct under Max-fill.
  - **Gallery name filter (`2cc24e4`).** Search box beside the sort control under the same "more than one save" gate. Tokens ANDed in any order ("star 8" finds "8-point Star") — a saved name is written for filing, not searching. Two calls worth keeping: the query is **not persisted** (a remembered filter greets the next visit with a near-empty grid and no visible cause), and a filter matching nothing renders **its own** empty state, never "Nothing saved yet" — telling someone their library is empty when it isn't is the worst thing that screen could say. The active filter is reported in bug capture, being the likeliest cause of a "my pattern is missing" report and invisible in a screenshot. ✅ Headless-verified end to end.
  - **The overlap-tile Strand "toggle" was cancelled, not built (`d821a41`).** Re-checking the code showed the 2026-06-14 toggle decision was superseded three days later by `d99c725`, which **deleted** the internal-edge gate rather than gating it. The wanted default ships by construction; what remained was an unrequested opt-in whose implementation means re-introducing deleted machinery — which the memo refuses twice.
  - **Tile-stamping cancelled (`f2914d0`)** — the user's call: a **Tile edges** line set with the base set's edge + vertex families off turns every Tile face into an ordinary Void, so stamps/fills/gradients/Combine/Reach all reach Tiles with no new code.
  - Tests **1680 green** (+15), tsc + build clean. Verification scripts kept under `scripts/verify/`.

**NEXT — pick one:**
0z. **Browser-verify the 2026-08-02 stamp session (5 commits, nothing in it has been seen in a browser).** Decoration → Stamp, on a pattern with stamps: (a) **Overlap** — `Clip to shape | Overlap` under the selected shape, and `Clipped`/`Overlap` on each stack row; needs Focus-mode zoom > 100% before anything spills. (b) **Stack** — "Stamp stack — front to back" list, ▲/▼ reorder, front-most first; re-uploading an image must NOT move a stamp in the list. (c) **Mirror** — `Reflect | Upright | Flipped`; Upright/Flipped should each make a whole class agree on handedness, oppositely. (d) **Focus mode `⇄ Flip`** — mirrors the image; the Rotation slider must still turn what you see.
0y. ⚠️ **Open question from that session: `4a0d2dd` (`canonicalSelfMirror`) showed NO visible change on the user's pattern.** Either their Void class is chiral (fallback path — plausible, the screenshot looked rosette-ish) or the self-mirror isn't firing. The three-way Mirror control made it moot for authoring, so this is diagnosis, not a blocker. **Offered and awaiting the user's word: build a headless render harness** (see `feedback_headless_browser_no_sudo`) so stamp changes can be *looked at* before shipping — three rounds this session went "reasoned correctly, looked wrong", each costing the user a screenshot.
0a. **Use the new Bug capture for the next report you file.** It exists precisely so the "⏳ browser-verify" items below stop needing a round trip: hit the bug button, write the note, hand over the Markdown or the JSON bundle. Possible follow-ups if it proves useful: whole-page (not just canvas) screenshots — needs a DOM-rasteriser dependency; a "load this report's config into the Lab" button; attaching the last N reducer actions.

0b. ~~**Browser-verify the Decoration Undo / Clear paint work (`65d09d9`).**~~ ✅ **USER-CONFIRMED 2026-08-05** — *"undo paint and clear is working"*. The data-loss half is closed: a paint-then-Ctrl+Z on a **legacy substrate** used to blank the canvas (the snapshot only covered the Patch, and restoring a `null` Patch clears the Lab), and the paired `HistorySnapshot = { editor, decoration }` fixed it.

0c. ~~**Browser-verify the download-naming work (`4241d39` + `536441b`).**~~ ✅ **HEADLESS-VERIFIED 2026-08-05** — all three rungs, script kept at `scripts/verify/verify3.mjs`: a preset exports as `Trihexagonal-3.6.3.6.svg` (dots preserved, per the `sanitiseFileBase` keep-dots decision), the same config after Save → "Kagome study" exports as `Kagome-study.svg` (**library name outranks the preset label**), and a from-scratch `New patch` exports as `islamic-pattern.svg`. Zero console errors. Original script below, kept for the manual pass: Save a tessellation as e.g. `Kagome study` → Export SVG / PNG / JSON ⇒ `Kagome-study.svg` etc. Then load a **preset** from the shelf (which clears the linked save) and export ⇒ the preset's label, e.g. `Small-Rhombitrihexagonal-3.4.6.4.svg`. Then `New` (from-scratch Patch, never saved) ⇒ `islamic-pattern.svg`. Also worth an eyeball: preset labels carry `{ } , ' ( ) @ °` (`Kepler's Star (4.4.4.4 @ 67.5°)`) — all legal in a filename, but confirm the browser doesn't mangle the download.
...
0. **Browser-verify `fa4e89d` (~5 min, any model) + answer the #34 question.** In the Lab, Builder → Configuration **4.6.12**, Design Phase. Delete or No-Seed one **square** Cell to open a gap. Set the **dodecagon** Cell's symmetry to **Full** (this is the step that makes it the internal active Cell). Complete-mode → click the square Cell's four Boundary corners. Expect **one** Tile, in the square, nothing scattered into the sibling Cells or the neighbour stamps. Control: empty the **dodecagon** instead, set its symmetry Full, complete a wedge from its centre to two adjacent corners ⇒ **12** wedges, all inside the dodecagon.
   ...
   **⚠️ PARTIALLY verified headless 2026-08-05 — `scripts/verify/c34.mjs` + its README.** On 4.8.8 with the square Cell at Symmetry **Full**, a Guide drawn across it lays down 4 linked orbit members hugging that Cell, and a triangle placed on an Anchor inside it lands at `(170.7, 149.6)` — 57 units from the square Cell centre `(120.71, 120.71)`, inside it. **The old bug's signature is absent** (nothing near the `(±198, ±198)` fling). **NOT established: the 8-image orbit** — the run commits exactly one Tile, which is correct for a **non-stamping** Anchor (world-space `guideTiles`), so the orbit branch may never have been entered. The script toggles "Stamp with Lattice" and confirms it reads true, but the Anchor it clicks is the Cell-centre crossing, whose stamp is the AND of both crossing Guides; whether both linked members flipped was not confirmed. **Next session: pick an Anchor that is NOT the Cell centre** (it is also D4's fixed point and collides with the seed Tile, so every size badges ⚠), confirm the Anchor's stamp state, then assert 8 images.

   **Same idea, second half — Place-on-Anchors (`a8af5b2`, #34, NEW 2026-08-03).** Configuration **4.8.8**, Design → **Construct**. Draw a Guide line crossing the **square** Cell with its **stamp** toggle **on**. Set the square Cell's symmetry to **Full**. Switch to **Place**, click an Anchor **inside the square**, pick a triangle. Expect **8 images hugging the square Cell**. The old bug orbited them about the world origin out to roughly (±198, ±198) — across and past the octagon. If you see that, the fix regressed. ~~Then: do we close #34 now or leave it folded into #30?~~ → **CLOSED**; it didn't need #30, `resolveHostCell` already existed.
1. **Browser-verify the Decoration/Stamp session (Sonnet/Opus) — 3 left, all reachable in one pass.** Load a saved pattern with curves on, go Decoration → Stamp:
   ...
   - ~~`2d4e504` **curved canvases**~~ — 10-second confirm only. Its Focus-mode half read the same dropped `keyPolygon` that `4ba619d` fixed, and the gradient Focus editor beside it is user-confirmed, so this is expected good. Open Stamp → Focus mode on a curved Void and tick it off.
   - ~~`88bd29d` curved gradients~~ — ✅ user-confirmed 2026-08-02.
2. ~~**Decide (c) — the 1.5° curve cliff.**~~ ✅ **DONE + BROWSER-VERIFIED 2026-08-02 (`a97e079`).** Closed out.

2b. ~~**#55 — no way to add or edit a Frame on a Gallery preset.**~~ ✅ **DONE 2026-08-03 (`6ea2ba7`), issue CLOSED, headless-verified.** Option 1 as written up on the issue; the question about which load path the user used turned out not to matter — the legacy substrate genuinely had no Frame UI and now has one. The 2026-08-02 trace was right in every particular. Residual, unchanged: `frameFieldBox` keys the Decoration field off the frame bbox, so dragging a legacy Frame re-keys the field per tick — same known wart as Patch frames (`project_decoration_stage_idea`).
2c. ~~(a) **Vector SVG export carries no background.**~~ ✅ **DONE 2026-08-05 (`63942ed`), headless-verified** — see the latest entry. (b) **Void stamps still abut** — the seam stroke closes fill-to-fill, but a `<clipPath>` can't be stroked, so a stamped Void hairlines against its neighbour. Reads far weaker (the seam shows the fill under the stamp, not the paper) *unless* the Void is unfilled. Fix would be inflating the clip polygon by a half-pixel equivalent, which is world-space and so zoom-dependent — the reason the fill fix used `non-scaling-stroke` instead.
3. **Answer #54 — the canonical-pose split (Opus).** Not cosmetic: **stamps currently render one image at two scales inside a single congruent class** on a curved field. Three options are written up on the issue; option 2 (chirality-symmetrised extent) fixes gradients fully but leaves stamps needing the pose fix. Wants a decision before code. **New input from 2026-08-02:** the same tie-break also decides each instance's *handedness*, and every populous class splits ~50/50 (measured, pinned in `stamps.test.ts`). A **fourth option** now exists — make the tie-break prefer the unreflected traversal when the tied set holds one — which would kill the gratuitous mirroring on symmetric shapes at the source. Deliberately NOT taken: the pose is shared with gradients, so it moves existing saved work. Fold it into the #54 decision rather than deciding it separately.
4. ~~**Issue hygiene.**~~ ✅ **DONE 2026-08-03.** #44 / #45 / #46 / #42 / #28 all closed with verifying-commit pointers. **#40 scoped** in the same pass — see the latest entry and the issue comment; the headline is that the α > interior-half-angle predictor is exact (88/88, zero misses), 20 of 26 tilings are affected, it reaches θ=71° on sharp tiles, and the "clamp the slider per shape" option is ruled out by per-vertex partial leaks. Remaining open issues worth a look: **#41** (Morph C1 kinks) and **#52** (archimedes-star 12-gon — needs YOUR call: fix the tile data, fix PIC, or both).
5. ~~**Guides slices 4–6.**~~ ✅ **v1 COMPLETE 2026-08-03** — #29 (`ffd9a6b`/`e2295b6`) and #30 (`7ffcb86`) both shipped, browser-verified and closed, joining #26–#28/#31/#33/#34. **Only #32 (Girih preset reveal) is left on the `guides` label, and it is the NEXT AGREED TASK (user, 2026-08-03).** ⚠️ **It is not implementable as written** — read it before opening an editor. Three things stand in the way: (a) the ticket itself says *"too vague to implement yet — next step is a short grill/enumeration pass once v1 has been used in anger"*; (b) **Girih presets are tier-3 view-only** (`rosette-patch`, legacy render path — there is **no Patch to hang Guides on**), so either tier-3 Patch conversion lands first or Guides must additionally render on the legacy preset path — that fork is the real decision; (c) **content authoring** — someone has to author a historically-correct Guide set per preset, and the ticket's own plan was that the first content falls out of the user authoring a Girih layout with v1, which has not happened yet. ⚠️ **Model:** the issue asks for **Fable** for the grill/enumeration pass; this session ran **Opus 5 / high**. Per the standing rule, **state the recommendation and ask before starting** if the active model differs. ~~**#34**~~ ✅ **FIXED + CLOSED 2026-08-03 (`a8af5b2`)** — it did NOT need to fold into #30, `resolveHostCell` already existed. Its "rendering is unaffected" caveat turned out to be false under symmetry (see the latest entry). Spec: `CONSTRUCTION_GUIDES_SPEC.md` + ADR-0008.
6. **Decoration stamps v2 (Sonnet/Opus).** v1 ships for Voids only; Overlap + stacking order + the Mirror ladder + Focus-mode Flip landed 2026-08-02. Remaining = ladder rungs (cell/patch/instance) + asset library. ~~**Tile**-stamping~~ **CANCELLED 2026-08-05 (user's call).** The Composition Phase already reaches it: add a **Tile edges** line set (`FigureControls.tsx:480`) and switch the base set's **Edge strands** + **Vertex strands** off (`reducer.ts:244-251` allows both off exactly when an emitting extra set exists) ⇒ each Tile face becomes an ordinary **Void**, so stamps, fills, gradients, Combine and the whole Reach ladder apply to Tiles with **no new code**. A dedicated Tile path would have been a second identity/keying system for shapes the Void system can already name. **Generalise before building any Decoration feature "for Tiles": check whether a line-set change turns those Tiles into Voids first.** Memory: `project_decoration_stamps_idea.md`.
7. **Roadmap design findings still open (Opus).** (a) **Pass-4 right-side Inspector** is blocked by the selection-state split (Canvas-local vertex state) + duplicated per-mode shell — the old "3rd-column-ready grid" claim is FALSE. (b) **Builder Configuration registry** — 7-site ladder to add one, with a silent square-basis fallback in `compositionCellBasis`. Memory: `project_thermonuclear_review_round2.md`.
8. ~~**Open bugs (unscheduled).** Force-overlapped Tiles emit their own Strands.~~ ✅ **NOT OPEN — re-checked against the code 2026-08-05.** The 2026-06-14 "make it a toggle" decision was superseded on 2026-06-17 by `d99c725`, which **deleted** the mechanism instead of gating it (`buildInternalEdgeSet`, `runPIC`'s `edgeContext`, `usePattern`'s `neighbourEdgeContext` are all gone — the only surviving mention is a comment in `vertexStrandsPeriodic.test.ts` recording their removal). The wanted default ships by construction: PIC iterates real Tiles only, so an overlap region is never its own Tile (`pic/index.ts:660`), pinned by `vertexStrandsOverlap.test.ts`. What is left is an **opt-in feature nobody has asked for** — overlap-region-as-its-own-Figure — and building it means re-introducing deleted machinery. The memo refuses this twice ("don't build the gate until confirmed"). Revisit only on an explicit request. Memory: `project_overlap_tiles_strand_bug.md`.
9. ~~**Gallery name filter.**~~ ✅ **DONE 2026-08-05 (`2cc24e4`), headless-verified.** `filterSaves` in `gallerySort.ts` (whose remit is now "which saves the grid shows, and in what order").
