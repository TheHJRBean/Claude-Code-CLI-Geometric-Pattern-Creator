...
A Shape-type Frame can also act as a **completion boundary**: its edge is divided into **Frame nodes** spaced one seed `edgeLength` apart, and the pattern is tiled *out to* the Frame using the **Complete** operation, so the pattern meets the frame in clean, tile-aligned fashion rather than being sliced. Any mid-edge remainder is absorbed by Complete's irregular fallback; hard **clip** applies only to whatever can't be tiled. The n-ring type has no gap, so it never completes — it only clips. Boundary-completion Tiles are frame-scoped (they don't repeat under the **Lattice**) and PIC runs over them so **Strands** flow to the frame edge.
_Avoid_: crop, mask, clip, window (those are mechanisms or metaphors, not the noun); "Tiling frame" / "Shape frame" (rejected — one noun, Frame, with a type); bare "node" (use **Frame node** for the seed-spaced division points)
