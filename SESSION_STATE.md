...
> **Metrics snapshot (2026-06-13, end of session):** tests **315 → 382** (+67 across chunks 2/3/4/5). Largest-file table (vs Tier-0 baseline): TessellationLabMode 2243→812, Canvas 974→922, Sidebar 907→890, reducer 877 (unchanged), usePattern 902 (unchanged). Bundle JS 418.96 kB (gzip 127.23). No file >1000 except none. FPS not re-measured this session (no perf-touching change).
>
...
- **▶ NEXT (cold-start recipe for a fresh session) — Chunk 6 (`tilings/tapratsTiling.ts`, 796 ln).** It **already has tests** (`tapratsTiling.test.ts`) and is lower behavioural risk → this is a **Std-0/4 audit + adversarial coverage**, not a restructure. Steps: (a) `git checkout -b review/tapratsTiling`; (b) read the file cold + its existing test; apply Std 0 (magic constants), Std 4 (generic mechanisms hiding data-shape assumptions); (c) add adversarial tests at tiling boundaries (degenerate/empty/edge-case vertex configs) — confirm they pass on current behaviour; (d) only restructure if a *genuine* code-judo win appears (don't force one — see the "theme" note above); (e) verify `npx tsc --noEmit && npx vitest run && npm run build`, all green; (f) update the ledger (add a Chunk-6 section + result) + this resume pointer; (g) commit on the branch, then `git checkout main && git merge --ff-only review/tapratsTiling && git push && git branch -d review/tapratsTiling`. Per-chunk checklist lives in `THERMONUCLEAR_REVIEW_PLAN.md` §4.
  - **After Chunk 6:** Tier-2 logic — Chunk 7 (`pic/` — known minefield, characterization tests MANDATORY before touching `emitStarArms`; folds in the OPEN `project_overlap_tiles_strand_bug`), Chunk 8 (`editor/` placement family), Chunk 9 (`decoration/`). Then Tier-3 test-free zones (10 rendering / 11 components / 12 types) + Tier-4 dup sweep (13).
