...
**Latest (2026-08-05, Strands — "Apply to all Tiles"): SHIPPED + ✅ HEADLESS-VERIFIED.** User: *"a toggle for the strand editing features that is 'apply to all tiles' so if I change the angle of one tile, it changes it for the rest in the config."*
  ...
  - UI: one checkbox at the head of the Strands section, shown only from two Tile types up, + a live "all N Tile types" hint. Reported in bug capture. Headless-verified on 4.8.8 (`scripts/verify/applyToAllTiles.mjs`): toggle **off** ⇒ Octagon 13° / Square 67.5° (diverged); toggle **on** ⇒ both read 75.5° off one drag. Tests **1704 green**, tsc + build clean.
