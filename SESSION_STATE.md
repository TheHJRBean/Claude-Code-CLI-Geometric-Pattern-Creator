...
2c. **Two follow-ups the 2026-08-03 seam/background session knowingly left open (small, either model).** (a) **Vector SVG export carries no background.** The colour rides only as the root `<svg style="background:…">`; browsers honour it, Illustrator/Inkscape generally don't — now that the background is *user-editable*, an SVG export silently loses a deliberate choice. Fix = a `<rect>` underlay in the exported markup, which changes every existing export, so it wants a nod first. Memory: `project_export_view_fit_idea`. (b) **Void stamps still abut** — the seam stroke closes fill-to-fill, but a `<clipPath>` can't be stroked, so a stamped Void hairlines against its neighbour. Reads far weaker (the seam shows the fill under the stamp, not the paper) *unless* the Void is unfilled. Fix would be inflating the clip polygon by a half-pixel equivalent, which is world-space and so zoom-dependent — the reason the fill fix used `non-scaling-stroke` instead.
