 ...
 * record change. Only PAINTED Voids get a fill; unpainted Voids stay
 * transparent. The across-frame gradient (#45) is no longer a per-Void
 * underlay: it renders as a single full-viewport **background wash** behind
 * the whole composition (`PatternSVG` `frame-gradient-bg` rect), so a
 * continuous wash shows through every unpainted Void AND the transparent tiles,
 * edge-to-edge — painted Voids simply cover it. */
